...
class UnitextDocument(UnitextItem):
    ...
    def clone(self) -> 'UnitextItem':
        ...
        if (self.header is not None): 
            res.header = self.header.clone()
        if (self.footer is not None): 
            res.footer = self.footer.clone()
    ...
    def get_plaintext(self, res : io.StringIO, pars : 'GetPlaintextParam'=None) -> None:
        ...
        if (self.source_plain_text is not None): 
            ...
            return
        if (pars is None): 
            pars = GetPlaintextParam._new326(True)
    ...
    def get_html(self, res : io.StringIO, par : 'GetHtmlParam'=None) -> None:
        ...
        if (par.out_header_and_footer): 
            if (self.header is not None): 
                print("\r\n<HR/><H2>Верхний колонтитул</H1>", end="", file=res)
                self.header.get_html(res, par)
            if (self.footer is not None): 
                print("\r\n<HR/><H2>Нижний колонтитул</H1>", end="", file=res)
                self.footer.get_html(res, par)
    ...
    def get_xml(self, xml0_ : XmlWriter) -> None:
        ...
        if (self.content is not None): 
            ...
            xml0_.write_end_element()
        if (self.header is not None): 
            xml0_.write_start_element("header")
            self.header.get_xml(xml0_)
            xml0_.write_end_element()
        if (self.footer is not None): 
            xml0_.write_start_element("footer")
            self.footer.get_xml(xml0_)
            xml0_.write_end_element()
        for d in self.inner_documents: 
            d.get_xml(xml0_)
        xml0_.write_end_element()
        ...
        for x in xml0_: 
            if (Utils.getXmlLocalName(x) == "content"): 
                ...
                try: 
                    ...
                    pass
            elif (Utils.getXmlLocalName(x) == "header"): 
                for xx in x: 
                    self.header = UnitextHelper.create_item(xx)
                    break
            elif (Utils.getXmlLocalName(x) == "footer"): 
                for xx in x: 
                    self.footer = UnitextHelper.create_item(xx)
                    break
                ...
                except Exception as ex497: 
                    pass
        self.get_all_items(None, 0)
    
    def _optimize(self, is_content : bool, pars : 'CreateDocumentParam') -> 'UnitextItem':
        if (self.content is not None): 
            self.content = self.content._optimize(True, pars)
        if (self.header is not None): 
            self.header = self.header._optimize(True, pars)
        if (self.footer is not None): 
            self.footer = self.footer._optimize(True, pars)
        for d in self.inner_documents: 
            d._optimize(False, pars)
        return self
    ...
    def implantate_block(self, begin_head : int, begin_body : int, begin_tail : int, end : int, plaintext : str, parent_ : 'UnitextDocblock'=None) -> 'UnitextDocblock':
        ...
        """
        res = None
        if ((parent_ is not None and parent_.body is not None and begin_head >= parent_.begin_char) and end <= parent_.end_char): 
            wrapres498 = RefOutArgWrapper(res)
            cnt = UnitextImplantateHelper.implantate_block(parent_.body, begin_head, begin_body, begin_tail, end, plaintext, None, wrapres498)
            res = wrapres498.value
            if (res is not None): 
                parent_.body = cnt
        else: 
            wrapres499 = RefOutArgWrapper(res)
            cnt = UnitextImplantateHelper.implantate_block(self.content, begin_head, begin_body, begin_tail, end, plaintext, None, wrapres499)
            res = wrapres499.value
            if (res is not None): 
                self.content = cnt
        return res
    ...
    def remove_item_by_id(self, id0__ : str) -> bool:
        ...
        if (self.content is None): 
            return False
        cou = 0
        wrapcou500 = RefOutArgWrapper(cou)
        self.content = UnitextImplantateHelper.clear(self.content, id0__, None, UnitextContainerType.UNDEFINED, None, wrapcou500)
        cou = wrapcou500.value
        return cou > 0
    
    def remove_all_hyperlinks(self, sub_string : str=None) -> None:
        ...
        for it in its: 
            ...
            if (len(substrs) > 0): 
                ...
                k = 0
                while k < len(substrs): 
                    if (substrs[k] in hl.href.upper()): 
                        break
                    k += 1
                if (k >= len(substrs)): 
                    continue
            own = Utils.asObjectOrNull(hl.parent, UnitextContainer)
            if (own is None): 
                continue
            i = Utils.indexOfList(own.children, hl, 0)
            if (i < 0): 
                continue
            cnt = UnitextContainer._new501(hl.parent)
            own.children[i] = (cnt)
            for ch in hl.children: 
                cnt.children.append(ch)
                ch.parent = (cnt)
            change += 1
        if (change > 0): 
            self._optimize(False, CreateDocumentParam())
    ...
    def generate_ids(self) -> None:
        ...
        for it in all0_: 
            if (it._inner_tag is not None and it.id0_ is not None and it.id0_.startswith(it._inner_tag)): 
                n = 0
                wrapn502 = RefOutArgWrapper(0)
                inoutres503 = Utils.tryParseInt(it.id0_[len(it._inner_tag):], wrapn502)
                n = wrapn502.value
                if (inoutres503): 
        ...
        for it in all0_: 
            if (it._inner_tag is not None and it != self and it.id0_ is None): 
                id0__ = 0
                wrapid504 = RefOutArgWrapper(0)
                inoutres505 = Utils.tryGetValue(ids, it._inner_tag, wrapid504)
                id0__ = wrapid504.value
                if (not inoutres505): 
                    id0__ = 1
    ...
    def serialize(self) -> bytearray:
        ...
        """
        tmp = io.StringIO()
        with XmlWriter.create_string(tmp, UnitextDocument._new_xml_writer_settings_506("UTF-8", False)) as xml0_: 
            self.get_xml(xml0_)
    ...
    def find_by_id(self, id0__ : str) -> 'UnitextItem':
        ...
        if (self.content is not None): 
            ...
            if ((res) is not None): 
                return res
        if (self.header is not None): 
            res = self.header.find_by_id(id0__)
            if ((res) is not None): 
                return res
        if (self.footer is not None): 
            res = self.footer.find_by_id(id0__)
            if ((res) is not None): 
                return res
    ...
    @staticmethod
    def _new61(_arg1 : str, _arg2 : 'FileFormat') -> 'UnitextDocument':
        res = UnitextDocument()
    ...
    @staticmethod
    def _new344(_arg1 : str, _arg2 : str) -> 'UnitextDocument':
        res = UnitextDocument()
        res.source_file_name = _arg1
    ...
    @staticmethod
    def _new345(_arg1 : 'FileFormat', _arg2 : str) -> 'UnitextDocument':
        res = UnitextDocument()
        res.source_format = _arg1
        res.error_message = _arg2
        return res
    ...
    @staticmethod
    def _new350(_arg1 : 'FileFormat', _arg2 : str) -> 'UnitextDocument':
        res = UnitextDocument()
        res.source_format = _arg1
    ...
    @staticmethod
    def _new352(_arg1 : str) -> 'UnitextDocument':
        res = UnitextDocument()
        res.source_plain_text = _arg1
        return res
    
    @staticmethod
    def _new404(_arg1 : 'FileFormat', _arg2 : typing.List[tuple]) -> 'UnitextDocument':
        res = UnitextDocument()
        res.source_format = _arg1
        res.attrs = _arg2
        return res
    ...
    @staticmethod
    def _new_xml_writer_settings_506(_arg1 : str, _arg2 : bool) -> XmlWriterSettings:
