...
class UnitextDocblock(UnitextItem):
    """ Cтруктурирующий блок из заголовочной части, тела и окончания.
    ...
    def get_html(self, res : io.StringIO, par : 'GetHtmlParam') -> None:
        ...
        if (Utils.compareStrings(Utils.ifNotNull(self.typname, ""), "Footnote", True) == 0): 
            return
        tag_ = "div"
        if (Utils.compareStrings(Utils.ifNotNull(self.typname, ""), "Indention", True) == 0): 
            tag_ = "span"
    ...
    def _optimize(self, is_content : bool, pars : 'CreateDocumentParam') -> 'UnitextItem':
        if (self.head is not None): 
            if (self.head in self.head.children): 
                pass
            self.head = (Utils.asObjectOrNull(self.head._optimize(False, pars), UnitextContainer))
        if (self.body is not None): 
            self.body = self.body._optimize(False, pars)
        if (self.tail is not None): 
            self.tail = (Utils.asObjectOrNull(self.tail._optimize(False, pars), UnitextContainer))
        return self
    ...
    def _append_child(self, it : 'UnitextItem') -> bool:
        ...
        if (self.body is not None): 
            ...
            if (cnt is None): 
                cnt = UnitextContainer._new494(self.body.begin_char)
    ...
    @staticmethod
    def _new359(_arg1 : int, _arg2 : int) -> 'UnitextDocblock':
        res = UnitextDocblock()
