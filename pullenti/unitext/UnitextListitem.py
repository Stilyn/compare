...
class UnitextListitem(UnitextItem):
    ...
    def _optimize(self, is_content : bool, pars : 'CreateDocumentParam') -> 'UnitextItem':
        if (self.prefix is not None): 
            self.prefix = self.prefix._optimize(True, pars)
        if (self.content is not None): 
            self.content = self.content._optimize(True, pars)
        if (self.content is not None and self.content.is_whitespaces): 
            self.content = (None)
        if (self.sublist is not None): 
            self.sublist = (Utils.asObjectOrNull(self.sublist._optimize(False, pars), UnitextList))
        return self
    ...
    @staticmethod
    def _new336(_arg1 : 'UnitextItem') -> 'UnitextListitem':
        res = UnitextListitem()
        res.content = _arg1
        return res
