...
class WordDocument:
        ...
        if (doc is not None): 
                ...
                if (len(cps) > 2 and gen is not None): 
                    ...
                    while True:
                        ...
                        if (not cp in self.__m_footnotes): 
                            fn = UnitextFootnote._new420(k == 1, ge.finish(True, None))
            ...
            if (len(cpsat) == len(cpsat2)): 
                ...
                while True:
                    ...
                    if (tmp.tell() < 1): 
                        continue
                    cmt = UnitextComment._new42(Utils.toStringStringIO(tmp))
    ...
    def __create_uni(self, gen : 'UnitextGen', blk : 'UnitextItem', i0 : int, p0 : int, p1 : int, txt : str, level : int) -> None:
        ...
        while True:
            ...
            if (p.is_list and gen is not None and not (isinstance(blk, UnitextList))): 
                ...
                if (data is not None and len(data) > 1): 
                    ...
                    if (ilfo >= 0 and (ilfo < len(self.__lfos))): 
                        wrapnum455 = RefOutArgWrapper(None)
                        Utils.tryGetValue(self.__m_num_styles, self.__lfos[ilfo].lsid, wrapnum455)
                        num = wrapnum455.value
            ...
            while True:
                ...
                sup = None
                is_sub = False
                wrapsup461 = RefOutArgWrapper(None)
                inoutres462 = Utils.tryGetValue(self.__m_super_texts, cp, wrapsup461)
                sup = wrapsup461.value
                if (inoutres462): 
                    pass
                else: 
                    wrapsup456 = RefOutArgWrapper(None)
                    inoutres457 = Utils.tryGetValue(self.__m_sub_texts, cp, wrapsup456)
                    sup = wrapsup456.value
                    if (inoutres457): 
                        is_sub = True
                if (sup is not None): 
                    if (cp == 84223): 
                    ...
                    if (not Utils.isNullOrEmpty(stxt)): 
                        gen.append(UnitextPlaintext._new50(stxt, (UnitextPlaintextType.SUB if is_sub else UnitextPlaintextType.SUP)), None, -1, False)
                    else: 
                        pass
                    ii += len(sup)
                    if (len(sup) > 0): 
                    ...
                    if (lev == 0 or ((lev == 1 and str0.startswith("TOC")))): 
                        ...
                        if (str0_ is not None): 
                            ...
                            if (hyperlink is not None): 
                                hr = UnitextHyperlink._new51(hyperlink)
                                hr.children.append(UnitextPlaintext._new49(str0_))
    ...
    def __load_paragraphs(self, word_document_stream : Stream, clx : 'Clx', plcf_bte_papx : 'PlcBtePapx') -> None:
        ...
        while i < len(plcf_bte_papx._apn_bte_papx): 
            ...
            er = False
            wraper464 = RefOutArgWrapper(False)
            papx_fkp = BasicTypesReader._read_papx_fkp(word_document_stream, wraper464)
            er = wraper464.value
            if (er): 
                self.__m_error = True
            j = 0
    ...
    def __expand_prm(self, prm : 'Prm', clx : 'Clx') -> typing.List['Prl']:
        if (prm._fcomplex): 
            ...
            sprm = 0
            wrapsprm465 = RefOutArgWrapper(0)
            inoutres466 = Utils.tryGetValue(SinglePropertyModifiers._prm0map, prm._isprm, wrapsprm465)
            sprm = wrapsprm465.value
            if (not inoutres466): 
    ...
    def __loadlstfs(self, word_document_stream : Stream, length : int) -> None:
        ...
        while i < cou: 
            ...
            while j < (((1 if lstf.fsimple_list else 9))): 
                ...
                mnem = None
                wrapmnem467 = RefOutArgWrapper(None)
                inoutres468 = Utils.tryGetValue(WordDocument.M_NUM_TYPES_MAP, lvl.nfc, wrapmnem467)
                mnem = wrapmnem467.value
                if (inoutres468): 
                    nl.type0_ = DocxToText._get_num_typ(mnem)
                else: 
