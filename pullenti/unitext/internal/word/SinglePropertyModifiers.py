...
class SinglePropertyModifiers:
    ...
    def _get_sprm_by_name(name : str) -> int:
        res = 0
        wrapres443 = RefOutArgWrapper(0)
        inoutres444 = Utils.tryGetValue(SinglePropertyModifiers._map2, name, wrapres443)
        res = wrapres443.value
        if (inoutres444): 
            return res
    ...
    def get_sprm_name(sprm : int) -> str:
        res = None
        wrapres445 = RefOutArgWrapper(None)
        inoutres446 = Utils.tryGetValue(SinglePropertyModifiers._map0_, sprm, wrapres445)
        res = wrapres445.value
        if (not inoutres446): 
            return None
        else: 
            return res
