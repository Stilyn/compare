...
class BasicTypesReader:
    ...
    def _read_clx(s : Stream, length : int) -> 'Clx':
        ...
        while clxt == Prc._default_clxt:
            read = 0
            wrapread368 = RefOutArgWrapper(0)
            prcs.append(BasicTypesReader.__read_prc_without_cltx(s, length - position, wrapread368))
            read = wrapread368.value
            position += read
            if (position >= length): 
                return None
            clxt = ReadUtils._read_byte(s)
    ...
    def __read_pcdt_without_clxt(s : Stream, length : int) -> 'Pcdt':
        ...
        wrapread369 = RefOutArgWrapper(read)
        pcdt._lcb = int.from_bytes(ReadUtils._read_exact_ref(s, ReadUtils._dword_size, wrapread369)[0:0+4], byteorder="little")
        read = wrapread369.value
    ...
    def __read_plc_pcd(s : Stream, length : int) -> 'PlcPcd':
        ...
        i = 0
        while i <= n: 
            wrapread370 = RefOutArgWrapper(read)
            plc_pcd._cps[i] = int.from_bytes(ReadUtils._read_exact_ref(s, ReadUtils._dword_size, wrapread370)[0:0+4], byteorder="little")
            read = wrapread370.value
            i += 1
    ...
    def _read_prls(s : Stream, length : int, read : int) -> typing.List['Prl']:
        ...
        while position < length:
            ...
            if ((length - position) == 1): 
                ...
                try: 
                    wrapread_portion371 = RefOutArgWrapper(0)
                    prl = BasicTypesReader.__read_prl(s, length - position, wrapread_portion371)
                    read_portion = wrapread_portion371.value
                    if (prl is None): 
                        break
                    prls.append(prl)
                except Exception as ex372: 
                    break
    ...
    def _read_papx_fkp(s : Stream, err : bool) -> 'PapxFkp':
        ...
        with MemoryStream(data) as data_stream: 
            ...
            while i < cpara: 
                ...
                try: 
                    wrapposition373 = RefOutArgWrapper(position)
                    grp_prl_and_istd._grpprl = BasicTypesReader._read_prls(data_stream, grpprl_in_papx_length - position, wrapposition373)
                    position = wrapposition373.value
    ...
    def _read_chpx_fkp(s : Stream) -> 'ChpxFkp':
        ...
        with MemoryStream(data) as data_stream: 
            ...
            while i < crun: 
                ...
                position = 0
                wrapposition374 = RefOutArgWrapper(position)
                chpx._grpprl = BasicTypesReader._read_prls(data_stream, grpprl_length - position, wrapposition374)
                position = wrapposition374.value
    ...
    def _read_stsh(s : Stream, stsh_length : int) -> 'STSH':
        ...
        stsh = STSH()
        read = 0
        wrapread378 = RefOutArgWrapper(read)
        cb_stshi = int.from_bytes(ReadUtils._read_exact_ref(s, ReadUtils._word_size, wrapread378)[0:0+2], byteorder="little")
        read = wrapread378.value
        read_portion = 0
        wrapread_portion377 = RefOutArgWrapper(0)
        stshi = BasicTypesReader.__read_stshi(s, cb_stshi, wrapread_portion377)
        read_portion = wrapread_portion377.value
        read += read_portion
        stsh._stshi = stshi
        stsh._rglpstd = Utils.newArray(stsh._stshi._stshif._cstd, None)
        i = 0
        while i < len(stsh._rglpstd): 
            wrapread376 = RefOutArgWrapper(read)
            cb_std = int.from_bytes(ReadUtils._read_exact_ref(s, ReadUtils._word_size, wrapread376)[0:0+2], byteorder="little")
            read = wrapread376.value
            if (cb_std > (0)): 
                wrapread_portion375 = RefOutArgWrapper(0)
                std = BasicTypesReader.__read_std(s, cb_std, stshi._stshif._cbstdbase_in_file, wrapread_portion375)
                read_portion = wrapread_portion375.value
                stsh._rglpstd[i] = std
                read += read_portion
            i += 1
        return stsh
    ...
    def __read_stk_paragrlpupx(s : Stream, length : int, read : int) -> 'StkParaGRLPUPX':
        ...
        wrapposition383 = RefOutArgWrapper(position)
        stk_paragrlpupx._upx_papx = BasicTypesReader.__read_upx_papx(s, length - position, wrapposition383)
        position = wrapposition383.value
        wrapposition382 = RefOutArgWrapper(position)
        stk_paragrlpupx._upx_chpx = BasicTypesReader.__read_upx_chpx(s, length - position, wrapposition382)
        position = wrapposition382.value
        if (position < length): 
            stk_para_upx_grlpupxrm = StkParaUpxGrLPUpxRM()
            wrapposition381 = RefOutArgWrapper(position)
            stk_para_upx_grlpupxrm._upx_rm = BasicTypesReader.__read_upx_rm(s, length - position, wrapposition381)
            position = wrapposition381.value
            wrapposition380 = RefOutArgWrapper(position)
            stk_para_upx_grlpupxrm._upx_papxrm = BasicTypesReader.__read_upx_papx(s, length - position, wrapposition380)
            position = wrapposition380.value
            wrapposition379 = RefOutArgWrapper(position)
            stk_para_upx_grlpupxrm._upx_chpxrm = BasicTypesReader.__read_upx_chpx(s, length - position, wrapposition379)
            position = wrapposition379.value
    ...
    def __read_stk_chargrlpupx(s : Stream, length : int, read : int) -> 'StkCharGRLPUPX':
        ...
        wrapposition386 = RefOutArgWrapper(position)
        stk_chargrlpupx._upx_chpx = BasicTypesReader.__read_upx_chpx(s, length - position, wrapposition386)
        position = wrapposition386.value
        if (position < length): 
            stk_char_upx_grlpupxrm = StkCharUpxGrLPUpxRM()
            wrapposition385 = RefOutArgWrapper(position)
            stk_char_upx_grlpupxrm._upx_rm = BasicTypesReader.__read_upx_rm(s, length - position, wrapposition385)
            position = wrapposition385.value
            wrapposition384 = RefOutArgWrapper(position)
            stk_char_upx_grlpupxrm._upx_chpxrm = BasicTypesReader.__read_upx_chpx(s, length - position, wrapposition384)
            position = wrapposition384.value
    ...
    def __read_stk_tablegrlpupx(s : Stream, length : int, read : int) -> 'StkTableGRLPUPX':
        ...
        position = 0
        wrapposition389 = RefOutArgWrapper(position)
        stk_tablegrlpupx._upx_tapx = BasicTypesReader.__read_upx_tapx(s, length - position, wrapposition389)
        position = wrapposition389.value
        wrapposition388 = RefOutArgWrapper(position)
        stk_tablegrlpupx._upx_papx = BasicTypesReader.__read_upx_papx(s, length - position, wrapposition388)
        position = wrapposition388.value
        wrapposition387 = RefOutArgWrapper(position)
        stk_tablegrlpupx._upx_chpx = BasicTypesReader.__read_upx_chpx(s, length - position, wrapposition387)
        position = wrapposition387.value
        read.value += position
    ...
    def __read_stk_listgrlpupx(s : Stream, length : int, read : int) -> 'StkListGRLPUPX':
        ...
        wrapposition390 = RefOutArgWrapper(position)
        stk_listgrlpupx._upx_papx = BasicTypesReader.__read_upx_papx(s, length - position, wrapposition390)
        position = wrapposition390.value
        read.value += position
