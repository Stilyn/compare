...
class TiffHelper:
    ...
    class Tiff:
        ...
        def store(self, page_index : int, fout : Stream) -> None:
            ...
            first_pass560 = True
            while True:
                if first_pass560: first_pass560 = False
                else: i += 1
        ...
        def get_page_text(self, page_index : int) -> str:
            ...
            first_pass561 = True
            while True:
                if first_pass561: first_pass561 = False
                else: i += 1
                if (not (i < dircount)): break
                self.__m_stream.position = baseoff + (2) + (i * 12)
                tag = self.__read_int(2)
    ...
    def create_doc(content : bytearray) -> 'UnitextDocument':
        ...
        with MemoryStream(content) as mem: 
            try: 
                ...
                has_txt = False
                if (ti.pages_count == 1): 
                    p = UnilayPage._new149(content)
                    wrapwi150 = RefOutArgWrapper(0)
                    wraphei151 = RefOutArgWrapper(0)
                    wrapdpx152 = RefOutArgWrapper(0)
                    wrapdpy153 = RefOutArgWrapper(0)
                    wrapm154 = RefOutArgWrapper(False)
                    wraphas_txt155 = RefOutArgWrapper(False)
                    ti.get_page_info(0, wrapwi150, wraphei151, wrapdpx152, wrapdpy153, wrapm154, wraphas_txt155)
                    wi = wrapwi150.value
                    hei = wraphei151.value
                    dpx = wrapdpx152.value
                    dpy = wrapdpy153.value
                    m = wrapm154.value
                    has_txt = wraphas_txt155.value
                    p.width = wi
                    p.height = hei
                    res.pages.append(p)
                else: 
                    ...
                    while i < ti.pages_count: 
                        with MemoryStream() as pm: 
                            ti.store(i, pm)
                            p = UnilayPage._new149(pm.toarray())
