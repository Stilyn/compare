...
class MyXmlReader:
    ...
    def create(data : bytearray) -> 'MyXmlReader':
        res = MyXmlReader()
        hlen = 0
        wraphlen94 = RefOutArgWrapper(0)
        enc = EncodingWrapper.check_encoding(data, wraphlen94)
        hlen = wraphlen94.value
        if (enc is None): 
    ...
    def read(self) -> bool:
        ...
        while self.__m_pos < len(self.__m_text): 
            ...
            nam = None
            val = None
            wrapnam95 = RefOutArgWrapper(None)
            wrapval96 = RefOutArgWrapper(None)
            inoutres97 = self.__read_attr(wrapnam95, wrapval96)
            nam = wrapnam95.value
            val = wrapval96.value
            if (inoutres97): 
