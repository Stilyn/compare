...
class UnitextHtmlGen:
    ...
    class MsoListItem:
        ...
        @staticmethod
        def _new56(_arg1 : str) -> 'MsoListItem':
    ...
    def create(self, nod : 'HtmlNode', dir_name : str, images : typing.List[tuple], pars : 'CreateDocumentParam') -> 'UnitextDocument':
        ...
        if (pars.load_document_structure): 
            ...
            if (s is not None): 
                ...
                if (len(s.head) > 0): 
                    ...
                    doc.header = gen.finish(True, None)
                if (len(s.tail) > 0): 
                    gen = UnitextGen()
                    for n in s.tail: 
                        self.get_uni_text(n, gen, None, 0)
                    doc.footer = gen.finish(True, None)
        ...
        if (ti is not None and ti.text is not None and doc.header is None): 
            gen = UnitextGen()
            gen.append_text(ti.text.strip(), False)
            doc.header = gen.finish(True, None)
                        ...
                        if (not nam in self.__m_anno): 
                            ...
                            if (it is not None): 
                                ...
                                if (tmp.tell() > 0): 
                                    ...
                                    self.__m_anno[nam] = UnitextComment._new42(Utils.toStringStringIO(tmp).strip())
                    else: 
                        gg = UnitextGen()
                        self.get_uni_text(nod, gg, None, 0)
                        fn = UnitextFootnote()
    ...
    def get_uni_text(self, nod : 'HtmlNode', res : 'UnitextGen', blk : object, lev : int) -> None:
        ...
        if (tag is not None and tag.name == "TR"): 
            ...
            first_pass550 = True
            while True:
                if first_pass550: first_pass550 = False
                else: ii += 1
                ...
                if (ch.get_attribute("ROWSPAN") is not None): 
                    wraprowsp43 = RefOutArgWrapper(0)
                    inoutres44 = Utils.tryParseInt(ch.get_attribute("ROWSPAN"), wraprowsp43)
                    rowsp = wraprowsp43.value
                    if (inoutres44): 
                        if (rowsp > 65534): 
                            rowsp = 65534
                        cel.row_span = rowsp
                if (ch.get_attribute("COLSPAN") is not None): 
                    wrapcolsp45 = RefOutArgWrapper(0)
                    inoutres46 = Utils.tryParseInt(ch.get_attribute("COLSPAN"), wrapcolsp45)
                    colsp = wrapcolsp45.value
                    if (inoutres46): 
                        if (colsp < 100): 
                            cel.col_span = colsp
                if (colsp <= 1): 
        ...
        if (tag is not None and tag.name == "COLGROUP"): 
            ...
            first_pass551 = True
            while True:
                if first_pass551: first_pass551 = False
                else: ii += 1
                if (not (ii < len(nod.children))): break
                ch = nod.children[ii]
                tag2 = HtmlTag.find_tag(ch.tag_name)
        ...
        if (tag is not None and ((tag.name == "OL" or tag.name == "UL"))): 
            ...
            li_typ = None
            if (tag.name == "OL"): 
                wrapli_num47 = RefOutArgWrapper(0)
                inoutres48 = Utils.tryParseInt(Utils.ifNotNull(nod.get_attribute("START"), ""), wrapli_num47)
                li_num = wrapli_num47.value
                if (not inoutres48): 
                    li_num = 1
            ...
            first_pass552 = True
            while True:
                if first_pass552: first_pass552 = False
                else: ii += 1
                if (not (ii < len(nod.children))): break
                ch = nod.children[ii]
                tag2 = HtmlTag.find_tag(ch.tag_name)
        ...
        if (tag is not None and ((tag.name == "SUP" or tag.name == "SUB"))): 
            ...
            if (len(tt) > 0 and (len(tt) < 10)): 
                res.append(UnitextPlaintext._new50(tt, (UnitextPlaintextType.SUB if tag.name == "SUB" else UnitextPlaintextType.SUP)), None, -1, False)
                return
        if (res is None): 
            ii = 0
            while ii < len(nod.children): 
        ...
        if (tag_name == "A"): 
            ...
            if (val is not None): 
                fn = None
                wrapfn52 = RefOutArgWrapper(None)
                inoutres53 = Utils.tryGetValue(self.__m_foots, val[1:], wrapfn52)
                fn = wrapfn52.value
                if (val.startswith("#") and inoutres53): 
                ...
                it = gg.finish(True, None)
                if (it is not None): 
                    hr = UnitextHyperlink._new51(val)
        ...
        if (not Utils.isNullOrEmpty(nod.text)): 
            ...
            first_pass553 = True
            while True:
                if first_pass553: first_pass553 = False
                else: ii += 1
                if (not (ii < len(nod.children))): break
                ch = nod.children[ii]
                ...
                if (li is not None): 
                    ...
                    if (li.level == (len(lists) + 1) and len(lists[0].items) > 0): 
                        ...
                        if (last.sublist is None): 
                            last.sublist = UnitextList._new55(li.level - 1)
    ...
    def __try_parse_list(self, nod : 'HtmlNode', lev : int) -> 'MsoListItem':
        ...
        if (not Utils.startsWithString(fi[2], "LFO", True)): 
            return None
        res = UnitextHtmlGen.MsoListItem._new56(fi[2])
        wrapi58 = RefOutArgWrapper(0)
        inoutres59 = Utils.tryParseInt(fi[1][5:], wrapi58)
        i = wrapi58.value
        if (not Utils.startsWithString(fi[1], "level", True) or not inoutres59): 
            return None
        res.level = i
        if (i < 1): 
            return None
        ...
        first_pass554 = True
        while True:
            if first_pass554: first_pass554 = False
            else: i += 1
            ...
            if (tmp.tell() > 0): 
                res.item.prefix = (UnitextPlaintext._new49(Utils.toStringStringIO(tmp).strip()))
            gg = UnitextGen()
