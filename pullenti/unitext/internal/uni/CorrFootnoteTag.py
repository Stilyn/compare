...
class CorrFootnoteTag:
    ...
    def try_parse(text : str, i : int, for_contaiter : bool, robust : bool=False) -> 'CorrFootnoteTag':
        ...
        if (text[i.value] != '<'): 
            if (robust and text[i.value] == '*'): 
                res1 = CorrFootnoteTag._new305(CorrFootnoteTyps.STARS)
                while i.value < len(text): 
                    if (text[i.value] != '*'): 
                        i.value -= 1
                        break
            ...
            if (((((i.value + 10) < len(text)) and text[i.value] == 'С' and text[i.value + 1] == 'н') and text[i.value + 2] == 'о' and text[i.value + 3] == 'с') and text[i.value + 4] == 'к' and text[i.value + 5] == 'а'): 
                ...
                while j < len(text): 
                    if (text[j] == ':' and num > 0): 
                        res1 = CorrFootnoteTag._new306(CorrFootnoteTyps.DIGIT, num)
                        i.value = j
                        return res1
            ...
            if (str.isdigit(text[i.value]) and for_contaiter): 
                ...
                while j < len(text): 
                    if (str.isdigit(text[j])): 
                        ...
                        res1 = CorrFootnoteTag._new306(CorrFootnoteTyps.VERYDOUBT, num)
                        i.value = j
                        return res1
                    else: 
                        break
        ...
        if (text[ii] != '*' and not str.isdigit(text[ii])): 
            return None
        res = CorrFootnoteTag._new305((CorrFootnoteTyps.STARS if text[ii] == '*' else CorrFootnoteTyps.DIGIT))
        while ii < len(text): 
    ...
    @staticmethod
    def _new283(_arg1 : int, _arg2 : 'CorrFootnoteTyps') -> 'CorrFootnoteTag':
        res = CorrFootnoteTag()
        res.number = _arg1
        res.typ = _arg2
        return res
    ...
    @staticmethod
    def _new305(_arg1 : 'CorrFootnoteTyps') -> 'CorrFootnoteTag':
        res = CorrFootnoteTag()
        res.typ = _arg1
        return res
    
    @staticmethod
    def _new306(_arg1 : 'CorrFootnoteTyps', _arg2 : int) -> 'CorrFootnoteTag':
        res = CorrFootnoteTag()
        res.typ = _arg1
        res.number = _arg2
        return res