...
class CorrTable:
    ...
    def try_parse(lines : typing.List['CorrLine'], i : int, nexti : int) -> 'CorrTable':
        ...
        while j < width: 
            ...
            first_pass594 = True
            while True:
                if first_pass594: first_pass594 = False
                else: r += 1
        ...
        while j < height: 
            ...
            first_pass595 = True
            while True:
                if first_pass595: first_pass595 = False
                else: w += 1
        ...
        if (rnum < 3): 
            return None
        tab = CorrTable._new318(i, (i + height) - 1)
        ...
        while h < len(rnums): 
            ...
            first_pass596 = True
            while True:
                if first_pass596: first_pass596 = False
                else: c += 1
                ...
                while hh <= h1: 
                    ...
                    first_pass597 = True
                    while True:
                        if first_pass597: first_pass597 = False
                        else: ww += 1
                        if (not (ww <= w1)): break
                        if (ww < 0): 
                            continue
                    ...
                    for jj in range(tmp.tell() - 1, -1, -1):
                        if (Utils.getCharAtStringIO(tmp, jj) != ' '): 
                            ...
                            break
                    hh += 1
                cel = tab.result.add_cell(rnums[h0], rnums[h1], c, cc, UnitextPlaintext._new49(Utils.toStringStringIO(tmp).strip()))
                c = cc
            h += 1
        tab.result._optimize(False, None)
        return tab
    
    @staticmethod
    def _new318(_arg1 : int, _arg2 : int) -> 'CorrTable':
        res = CorrTable()
        res.begin_ind = _arg1
        res.end_ind = _arg2
        return res