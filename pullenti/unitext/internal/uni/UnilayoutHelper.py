...
class UnilayoutHelper:
    ...
    class LayLine(object):
        ...
        def can_be_equals_except_page_number(self, ll : 'LayLine') -> bool:
            ...
            first_pass598 = True
            while True:
                if first_pass598: first_pass598 = False
                else: i += 1
                ...
                while (i0 < len(b0.text)) and (i1 < len(b1.text)):
                    ...
                    if (ch0 == ch1): 
                        ...
                        continue
                    oo = False
                    while str.isdigit(b0.text[i0]):
                        i0 += 1
                        oo = True
                    while str.isdigit(b1.text[i1]):
                        i1 += 1
                        oo = True
                    if (not oo): 
                        return False
        ...
        for p in pages: 
            for r in p.rects: 
                if (r.text is not None): 
                    ...
                    first_pass599 = True
                    while True:
                        if first_pass599: first_pass599 = False
                        else: i += 1
    ...
    def create_content_from_pages(doc : 'UnitextDocument', after_ocr : bool) -> None:
            ...
            if (len(head_title) > 0): 
                ...
                for ht in head_title: 
                    for b in ht.boxes: 
                        b.ignored = True
                        if (b.text is not None): 
                            fcnt.children.append(UnitextPlaintext._new49(b.text))
                        elif (b.image_content is not None): 
                            fcnt.children.append(UnitextImage._new250(b.image_content))
        ...
        first_pass600 = True
        while True:
            if first_pass600: first_pass600 = False
            else: pn += 1
            ...
            while i < len(lines): 
                if (i > 0): 
                    ...
                    if (((h1 < (h2 * (0.75)))) or (((h1 * (0.75)) >= h2))): 
                        ...
                        cou = 3
                    cnt.children.append(UnitextNewline._new322(cou))
                l_ = lines[i]
                for b in l_.boxes: 
                    b.tag = None
                if (i == 13): 
                    ...
                    if (j > 0): 
                        ...
                        if (d > (char_width / (3))): 
                            ...
                            while sps > 1: 
                                str0_ = (str0_ + " ")
                                sps -= 1
                            cnt.children.append(UnitextPlaintext._new49(str0_))
                    if (r.text is not None): 
                        txt = UnitextPlaintext._new49(r.text)
                        ...
                        img = UnitextImage._new325(r.image_content, r)
    ...
    def __correct_page_lines(p : 'UnilayPage') -> typing.List['LayLine']:
        ...
        while cha and (kk < 3): 
            ...
            first_pass601 = True
            while True:
                if first_pass601: first_pass601 = False
                else: i -= 1
                if (not (i > 0)): break
                if (lines[i].y < lines[i - 1].y): 
                    j = 0
                    y = lines[len(lines) - 1].y
            ...
            first_pass602 = True
            while True:
                if first_pass602: first_pass602 = False
                else: i += 1
                if (not ((i < (len(lines) - 1)) and (i < 3))): break
                if (lines[i].y > lines[i + 1].y): 
                    j = 0
                    y = lines[i].y
                    j = (i + 1)
