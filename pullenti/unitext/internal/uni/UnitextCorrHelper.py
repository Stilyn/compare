...
class UnitextCorrHelper:
    ...
    def remove_page_break_numeration(doc : 'UnitextDocument') -> None:
        ...
        first_pass603 = True
        while True:
            if first_pass603: first_pass603 = False
            else: k += 1
            if (not (k < 2)): break
            pages = list()
            i = 0
            first_pass604 = True
            while True:
                if first_pass604: first_pass604 = False
                else: i += 1
                ...
                tmp_cnt.get_plaintext(tmp, GetPlaintextParam._new326(False))
                if (tmp.tell() == 0): 
                    continue
                page = CorrPageText._new327(Utils.toStringStringIO(tmp), k == 0, i0, i1)
                pages.append(page)
            if (len(pages) < 2): 
                continue
            ...
            if (tittext is not None): 
                if (k == 0 and doc.header is None): 
                    doc.header = (UnitextPlaintext._new49(tittext))
                elif (k == 1 and doc.footer is None): 
                    doc.footer = (UnitextPlaintext._new49(tittext))
            k = k1
        for i in range(len(cnt.children) - 1, -1, -1):
            ...
            if (pt is None): 
                continue
            n = 0
            wrapn330 = RefOutArgWrapper(0)
            inoutres331 = Utils.tryParseInt(pt.text.strip(), wrapn330)
            n = wrapn330.value
            if (not inoutres331): 
                continue
    ...
    def remove_false_new_lines(doc : 'UnitextDocument', replace_nbsp : bool) -> None:
        ...
        if (replace_nbsp and nbsp_count > 100 and nbsp_count > (sp_count * 2)): 
            ...
            if (doc.header is not None): 
                doc.header._correct(LocCorrTyp.NPSP2SP, None)
            if (doc.footer is not None): 
                doc.footer._correct(LocCorrTyp.NPSP2SP, None)
        if (cou == 0): 
            return
        ...
        first_pass605 = True
        while True:
            if first_pass605: first_pass605 = False
            else: i += 1
        ...
        for i in range(len(lines) - 1, 0, -1):
            if (lines[i].merge): 
                ...
                if (i0 <= i1): 
                    ...
                    cnt.children[i0] = (UnitextPlaintext._new49(" "))
        if (changed): 
            doc._optimize(False, None)
    ...
    def restore_text_footnotes(doc : 'UnitextDocument') -> None:
        ...
        first_pass606 = True
        while True:
            if first_pass606: first_pass606 = False
            else: i += 1
            if (not (i < len(cnt.children))): break
    ...
    def __restore_text_footnotes_in_table(tab : 'UnitextTable') -> None:
        ...
        while r < tab.rows_count: 
            ...
            first_pass607 = True
            while True:
                if first_pass607: first_pass607 = False
                else: c += 1
                ...
                first_pass608 = True
                while True:
                    if first_pass608: first_pass608 = False
                    else: cc += 1
    ...
    def __restore_tables(cnt : 'UnitextContainer') -> bool:
        ...
        while i < len(lines): 
            ii = 0
            wrapii333 = RefOutArgWrapper(0)
            tab = CorrTable.try_parse(lines, i, wrapii333)
            ii = wrapii333.value
            if (tab is not None): 
                tabs.append(tab)
                i = tab.end_ind
