...
class CorrPageText:
    ...
    def __init__(self, txt : str, top : bool) -> None:
        ...
        if (not Utils.isNullOrEmpty(self.head_text) and str.isdigit(self.head_text[0])): 
            ...
            if (i == len(self.head_text) or self.head_text[i] == ' '): 
                num = 0
                wrapnum310 = RefOutArgWrapper(0)
                inoutres311 = Utils.tryParseInt(self.head_text[0:0+i], wrapnum310)
                num = wrapnum310.value
                if (inoutres311): 
                    self.number = num
        ...
        if (not Utils.isNullOrEmpty(self.head_text) and str.isdigit(self.head_text[len(self.head_text) - 1])): 
            while True:
                ...
                if ((i < 0) or self.head_text[i] == ' '): 
                    num = 0
                    wrapnum312 = RefOutArgWrapper(0)
                    inoutres313 = Utils.tryParseInt(self.head_text[i + 1:], wrapnum312)
                    num = wrapnum312.value
                    if (inoutres313): 
                        if (self.number > 0): 
                            self.number2 = num
                        else: 
        ...
        if (top and not Utils.isNullOrEmpty(self.text) and str.isdigit(self.text[0])): 
            ...
            if (i >= len(self.text) or self.text[i] == '\r' or self.text[i] == '\n'): 
                num = 0
                wrapnum314 = RefOutArgWrapper(0)
                inoutres315 = Utils.tryParseInt(self.text[0:0+i], wrapnum314)
                num = wrapnum314.value
                if (inoutres315): 
                    self.number = num
                    if (i >= len(self.text)): 
                        self.text = ""
        ...
        if (not top and not Utils.isNullOrEmpty(self.text) and str.isdigit(self.text[len(self.text) - 1])): 
            ...
            if ((i < 0) or self.text[i] == '\r' or self.text[i] == '\n'): 
                num = 0
                wrapnum316 = RefOutArgWrapper(0)
                inoutres317 = Utils.tryParseInt(self.text[i + 1:], wrapnum316)
                num = wrapnum316.value
                if (inoutres317): 
                    self.number = num
                    if (i <= 0): 
                        self.text = ""
    ...
    def process(pts : typing.List['CorrPageText']) -> str:
        ...
        first_pass592 = True
        while True:
            if first_pass592: first_pass592 = False
            else: i += 1
        ...
        first_pass593 = True
        while True:
            if first_pass593: first_pass593 = False
            else: i += 1
    ...
    @staticmethod
    def _new327(_arg1 : str, _arg2 : bool, _arg3 : int, _arg4 : int) -> 'CorrPageText':
