...
class UnitextImplantateHelper:
    ...
    def clear(content : 'UnitextItem', id0_ : str, type_name : str, cnt_typ : 'UnitextContainerType', own : 'UnitextContainer', cou : int) -> 'UnitextItem':
        ...
        if (own is None): 
            return content
        ii = Utils.indexOfList(own.children, cnt, 0)
        if (ii < 0): 
            return content
        del own.children[ii]
    ...
    def __split_texts(content : 'UnitextItem', begin : int, end : int, own : 'UnitextContainer'=None) -> 'UnitextItem':
        ...
        if (cnt is not None): 
            ...
            first_pass610 = True
            while True:
                if first_pass610: first_pass610 = False
                else: i += 1
        ...
        if (txt is not None): 
            ...
            creat = False
            if (own is None): 
                own = UnitextContainer._new358(txt.parent, txt.begin_char, txt.end_char, "cnt0_{0}_{1}".format(txt.begin_char, txt.end_char))
                own.children.append(txt)
                creat = True
            ii = Utils.indexOfList(own.children, content, 0)
            if (ii < 0): 
                return content
            jj = ii
            del own.children[ii]
    ...
    def implantate(content : 'UnitextItem', impl : 'UnitextContainer', text : str, own : 'UnitextContainer'=None) -> 'UnitextItem':
        ...
        if (cnt is not None): 
            ...
            first_pass611 = True
            while True:
                if first_pass611: first_pass611 = False
                else: i += 1
                if (not (i < len(cnt.children))): break
                if (i == 49): 
                    pass
                ...
                if (impl.begin_char > ch.end_char): 
                    continue
                if (((isinstance(ch, UnitextFootnote)) or (isinstance(ch, UnitextTable)) or (isinstance(ch, UnitextList))) or (isinstance(ch, UnitextDocblock))): 
    ...
    @staticmethod
    def implantate_block(content : 'UnitextItem', begin_head : int, begin_body : int, begin_tail : int, end : int, text : str, own : 'UnitextContainer', res : 'UnitextDocblock') -> 'UnitextItem':
        if (content is None): 
            return None
        if (end < content.begin_char): 
            return content
        ...
        if (tab is not None): 
            ...
            while r < tab.rows_count: 
                ...
                first_pass612 = True
                while True:
                    if first_pass612: first_pass612 = False
                    else: c += 1
                    ...
                    if (cel.begin_char <= bh and bh <= cel.end_char and cel.row_begin == cel.row_end): 
                        if ((bh < bb) and cel.end_char > bh and bb > cel.end_char): 
                            res0 = UnitextDocblock._new359(bh, cel.end_char)
                            res0.head = UnitextContainer._new87(UnitextContainerType.HEAD)
                            ...
                            if (res.value is None): 
                                res.value = res0
                            return tab
                        cel.content = UnitextImplantateHelper.implantate_block(cel.content, begin_head, begin_body, begin_tail, end, text, None, res)
                    elif (cel.begin_char <= bb and bb <= cel.end_char): 
                        if ((bh < bb) and cel.end_char > bh and bb > cel.end_char): 
                            res0 = UnitextDocblock._new359(bh, cel.end_char)
                            res0.body = (UnitextContainer._new87(UnitextContainerType.HEAD))
        ...
        if (li is not None): 
            for l_ in li.items: 
                if (l_.begin_char <= begin_head and begin_head <= l_.end_char): 
                    if (l_.content is not None): 
                        l_.content = UnitextImplantateHelper.implantate_block(l_.content, begin_head, begin_body, begin_tail, end, text, None, res)
                    if (l_.sublist is not None): 
                        UnitextImplantateHelper.implantate_block(l_.sublist, begin_head, begin_body, begin_tail, end, text, None, res)
            return li
        cnt = Utils.asObjectOrNull(content, UnitextContainer)
        if (cnt is not None): 
            i = 0
            first_pass613 = True
            while True:
                if first_pass613: first_pass613 = False
                else: i += 1
                if (not (i < len(cnt.children))): break
                ch = cnt.children[i]
                ...
                if (begin_head > ch._get_plaintext_ns_pos(text)): 
                    if ((begin_head < ch.end_char) and (isinstance(ch, UnitextTable))): 
                        UnitextImplantateHelper.implantate_block(ch, begin_head, begin_body, begin_tail, end, text, None, res)
                        return content
                    continue
                ...
                if ((isinstance(ch, UnitextTable)) and ch.begin_char <= begin_head and (end < ch.end_char)): 
                    UnitextImplantateHelper.implantate_block(ch, begin_head, begin_body, begin_tail, end, text, None, res)
                    return content
                res.value = UnitextDocblock._new359(begin_head, end)
                head = UnitextContainer._new87(UnitextContainerType.HEAD)
                body = UnitextContainer()
                tail = UnitextContainer._new87(UnitextContainerType.TAIL)
                j = 0
                j = i
                while j < len(cnt.children): 
        ...
        if ((isinstance(content, UnitextPlaintext)) and (((isinstance(content.parent, UnitextTablecell)) or (isinstance(content.parent, UnitextListitem))))): 
            if ((content.begin_char == begin_body and begin_body == begin_head and end == begin_tail) and end == content.end_char): 
                res.value = UnitextDocblock._new359(begin_head, end)
