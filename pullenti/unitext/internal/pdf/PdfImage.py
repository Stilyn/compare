...
class PdfImage(PdfRect):
    ...
    def __init__(self, xobj : 'PdfDictionary') -> None:
        ...
        self.image_width = 0
        self.image_height = 0
        wi = xobj.get_object("Width")
        if (wi is not None): 
            self.image_width = (math.floor(wi.get_double()))
        hi = xobj.get_object("Height")
        if (hi is not None): 
            self.image_height = (math.floor(hi.get_double()))
        sub = Utils.asObjectOrNull(xobj.get_object("Filter"), PdfName)
        if (sub is None): 
            aaa = Utils.asObjectOrNull(xobj.get_object("Filter"), PdfArray)
            ...
            if (parms is not None): 
                ...
                if (width_ > 0 and height_ > 0): 
                    with MemoryStream() as mem: 
                        ...
                        tif.write_tiff(mem, width_, height_, (TiffHelper.CCITT_Types.G4 if k < 0 else ((TiffHelper.CCITT_Types.G3_2D if k == 0 else TiffHelper.CCITT_Types.G3_1D))), 300, 300, invert, xobj.data)
                        self.content = mem.toarray()
            return
        if (((sub is None or sub.name == "FlateDecode")) and self.image_width > 0 and self.image_height > 0): 
            bpi = xobj.get_object("BitsPerComponent")
            dparms = xobj.get_dictionary("DecodeParms", None)
            predict = 0
            if (dparms is not None): 
                pred = Utils.asObjectOrNull(dparms.get_object("Predictor"), PdfIntValue)
                if (pred is not None and pred.val > 1): 
                    predict = pred.val
                    if (predict >= 10 and (predict < 15)): 
                        pass
            colorsp = xobj.get_object("ColorSpace")
            color_map = None
            if (bpi is not None and ((bpi.get_double() == 8 or bpi.get_double() == 4 or bpi.get_double() == 1)) and colorsp is not None): 
                ...
                if (isinstance(colorsp, PdfName)): 
                    ...
                    if (dii is None and arr.items_count == 2): 
                        ...
                        if (ddd is not None and (isinstance(ddd.get_object("Alternate"), PdfName))): 
                            color_typ = ddd.get_object("Alternate").name
