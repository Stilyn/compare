...
class PdfFont:
    ...
    def __init__(self, dic : 'PdfDictionary') -> None:
        ...
        if (desc is not None and desc.is_type_item("Font")): 
            ...
            wlist = Utils.asObjectOrNull(desc.get_object("W"), PdfArray)
            if (wlist is not None): 
                i = 0
                first_pass564 = True
                while True:
                    if first_pass564: first_pass564 = False
                    else: i += 1
                    ...
                    if (isinstance(it1, PdfArray)): 
                        ...
                        while j < aaa.items_count: 
                            ...
                            if (not cod0 + j in self.__m_glypths): 
                                self.__m_glypths[cod0 + j] = PdfFontGlyph._new186(it2.get_double(), cod0 + j)
                    ...
                    while j <= cod1: 
                        if (not j in self.__m_glypths): 
                            self.__m_glypths[j] = PdfFontGlyph._new186(w, j)
                        j += 1
                    i += 2
        warr = Utils.asObjectOrNull(dic.get_object("Widths"), PdfArray)
        if (warr is not None): 
            fi_char = Utils.asObjectOrNull(dic.get_object("FirstChar"), PdfIntValue)
            la_char = Utils.asObjectOrNull(dic.get_object("LastChar"), PdfIntValue)
            if (fi_char is not None and la_char is not None): 
                ...
                first_pass565 = True
                while True:
                    if first_pass565: first_pass565 = False
                    else: i += 1
                    ...
                    if (it is None): 
                        continue
                    w = it.get_double()
                    self.__m_glypths[cod0 + i] = PdfFontGlyph._new186(w, cod0 + i)
        uni_dic = dic.get_dictionary("ToUnicode", None)
        diff = None
        denc = dic.get_dictionary("Encoding", None)
        if (denc is not None): 
            diff = (Utils.asObjectOrNull(denc.get_object("Differences"), PdfArray))
        ...
        if (font_descr is not None): 
            it = font_descr.get_object("Ascent")
            if (it is not None): 
                self.__m_ascent = it.get_double()
            it = font_descr.get_object("Descent")
            if (it is not None): 
                d = it.get_double()
                if (self.__m_ascent > d): 
                    self.__m_line_height = (self.__m_ascent - d)
        if (self.__m_line_height == 0): 
            bbox = Utils.asObjectOrNull(dic.get_object("FontBBox"), PdfArray)
        ...
        if (subtyp == "Type3"): 
            ...
            bbox = Utils.asObjectOrNull(dic.get_object("FontMatrix"), PdfArray)
        ...
        if (subtyp == "Type3"): 
            arr = Utils.asObjectOrNull(dic.get_object("FontMatrix"), PdfArray)
        ...
        while i < diff.items_count: 
            ...
            while j < diff.items_count: 
                ...
                if (nam is None): 
                    break
                uni = 0
                wrapuni191 = RefOutArgWrapper(0)
                inoutres192 = Utils.tryGetValue(PdfFont.__m_diffs, nam.name, wrapuni191)
                uni = wrapuni191.value
                if (inoutres192): 
                    gli = None
                    wrapgli189 = RefOutArgWrapper(None)
                    inoutres190 = Utils.tryGetValue(self.__m_glypths, co, wrapgli189)
                    gli = wrapgli189.value
                    if (not inoutres190): 
                        gli = PdfFontGlyph()
                        gli.code = co
    ...
    def __load_to_unicode(self, dic : 'PdfDictionary') -> None:
        ...
        first_pass566 = True
        while True:
            if first_pass566: first_pass566 = False
            else: i += 1
            ...
            first_pass567 = True
            while True:
                if first_pass567: first_pass567 = False
                else: c += 1
                ...
                if (isinstance(list0_[i], PdfStringValue)): 
                    ...
                    while j <= range1: 
                        gl = None
                        wrapgl194 = RefOutArgWrapper(None)
                        inoutres195 = Utils.tryGetValue(self.__m_glypths, j, wrapgl194)
                        gl = wrapgl194.value
                        if (not inoutres195): 
                            gl = PdfFontGlyph._new193(j)
                ...
                if (isinstance(list0_[i], PdfArray)): 
                    ...
                    if (arr.items_count == (((range1 - range0) + 1))): 
                        ...
                        while j < arr.items_count: 
                            ...
                            uni_char = PdfFont.__to_int(val, 0, -1)
                            gl = None
                            wrapgl197 = RefOutArgWrapper(None)
                            inoutres198 = Utils.tryGetValue(self.__m_glypths, j + range0, wrapgl197)
                            gl = wrapgl197.value
                            if (not inoutres198): 
                                gl = PdfFontGlyph._new193(j + range0)
    ...
    def __load_cmap_from_true_type(self, ttf : 'PdfDictionary') -> None:
        ...
        first_pass568 = True
        while True:
            if first_pass568: first_pass568 = False
            else: tabs -= 1; pos += 16
            if (not (tabs > 0)): break
    ...
    def __read_format0(self, dat : bytearray, pos : int, platform : int) -> None:
        ...
        while i < 256: 
            id0_ = dat[p]
            gli = None
            wrapgli200 = RefOutArgWrapper(None)
            inoutres201 = Utils.tryGetValue(self.__m_glypths, id0_, wrapgli200)
            gli = wrapgli200.value
            if (not inoutres201): 
                gli = PdfFontGlyph._new193(id0_)
    ...
    def __read_format4(self, dat : bytearray, pos : int, platform : int) -> None:
        ...
        while i < seg_count: 
            ...
            while cod <= ecs[i]: 
                ...
                id0_ &= 0xFFFF
                gli = None
                wrapgli203 = RefOutArgWrapper(None)
                inoutres204 = Utils.tryGetValue(self.__m_glypths, id0_, wrapgli203)
                gli = wrapgli203.value
                if (not inoutres204): 
                    gli = PdfFontGlyph._new193(id0_)
                    self.__m_glypths[id0_] = gli
                gli.char0_ = (chr(cod))
    ...
    def __read_format6(self, dat : bytearray, pos : int, platform : int) -> None:
        ...
        while i < cou: 
            ...
            gli = None
            wrapgli206 = RefOutArgWrapper(None)
            inoutres207 = Utils.tryGetValue(self.__m_glypths, id0_, wrapgli206)
            gli = wrapgli206.value
            if (not inoutres207): 
                gli = PdfFontGlyph._new193(id0_)
                self.__m_glypths[id0_] = gli
            cod = first + i
    ...
    def _extract_text(self, tobj : 'PdfObject', state : 'PdfTextState') -> 'PdfText':
        ...
        first_pass569 = True
        while True:
            if first_pass569: first_pass569 = False
            else: k += 1
            ...
            first_pass570 = True
            while True:
                if first_pass570: first_pass570 = False
                else: i += self.__m_code_size
                if (not (i < len(input0_))): break
                cod = PdfFont.__to_int(input0_, i, self.__m_code_size)
                gl = None
                wrapgl208 = RefOutArgWrapper(None)
                inoutres209 = Utils.tryGetValue(self.__m_glypths, cod, wrapgl208)
                gl = wrapgl208.value
                if (not inoutres209): 
