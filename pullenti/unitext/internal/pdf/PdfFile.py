...
class PdfFile(PdfObject):
    ...
    def open0_(self, file_name : str, file_content : bytearray=None) -> None:
        ...
        if (self.__read_xrefs()): 
            ...
            while True:
                ...
                if ((chr(ch)) >= '0' and (chr(ch)) <= '9'): 
                    wrapid169 = RefOutArgWrapper(0)
                    wrapv170 = RefOutArgWrapper(0)
                    is_obj = self.__m_stream.try_read_id_and_version(False, wrapid169, wrapv170)
                    id0__ = wrapid169.value
                    v = wrapv170.value
                if (is_obj): 
                    obj = (None)
                    try: 
                ...
                if (str0_ == "xref"): 
                    i1 = 0
                    str0_ = self.__m_stream.read_word(False)
                    wrapi1173 = RefOutArgWrapper(0)
                    inoutres174 = Utils.tryParseInt(str0_, wrapi1173)
                    i1 = wrapi1173.value
                    if (inoutres174): 
                        i2 = 0
                        str0_ = self.__m_stream.read_word(False)
                        wrapi2171 = RefOutArgWrapper(0)
                        inoutres172 = Utils.tryParseInt(str0_, wrapi2171)
                        i2 = wrapi2171.value
                        if (inoutres172): 
                            if (i2 > i1): 
                ...
                if (str0_ == "trailer"): 
                    ...
                    if (root_obj is None): 
                        root_obj = trailer.get_object("Root")
                    if (trailer.get_object("Encrypt") is not None): 
        ...
        if (self.root_object is None): 
            ...
            while i < len(self.__m_indirect_objs): 
                if (isinstance(self.__m_indirect_objs[i], PdfDictionary)): 
                    if (self.__m_indirect_objs[i].get_object("Linearized") is not None): 
                        is_linearized = True
                        break
                i += 1
    ...
    def get_object(self, id0__ : int) -> 'PdfObject':
        ...
        uid = 0
        v = 0
        wrapuid175 = RefOutArgWrapper(0)
        wrapv176 = RefOutArgWrapper(0)
        inoutres177 = self.__m_stream.try_read_id_and_version(False, wrapuid175, wrapv176)
        uid = wrapuid175.value
        v = wrapv176.value
        if (not inoutres177): 
            return None
        if (uid != id0__): 
    ...
    def __read_xrefs(self) -> bool:
        ...
        str0_ = self.__m_stream.read_word(True)
        disp = 0
        wrapdisp184 = RefOutArgWrapper(0)
        inoutres185 = Utils.tryParseInt(str0_, wrapdisp184)
        disp = wrapdisp184.value
        if (not inoutres185): 
            return False
        while True:
            ...
            while True:
                ...
                count = 0
                i = 0
                wrapid0182 = RefOutArgWrapper(0)
                str0_ = self.__m_stream.read_word(True)
                inoutres183 = Utils.tryParseInt(str0_, wrapid0182)
                id0 = wrapid0182.value
                if (not inoutres183): 
                    break
                wrapcount180 = RefOutArgWrapper(0)
                inoutres181 = Utils.tryParseInt(self.__m_stream.read_word(True), wrapcount180)
                count = wrapcount180.value
                if (not inoutres181): 
                    break
                i = 0
                while i < count: 
                    w = self.__m_stream.read_word(True)
                    wrapdisp178 = RefOutArgWrapper(0)
                    inoutres179 = Utils.tryParseInt(w, wrapdisp178)
                    disp = wrapdisp178.value
                    if (not inoutres179): 
                        break
            ...
            if (trailer.get_object("Encrypt") is not None): 
                self.encrypt = trailer.get_dictionary("Encrypt", None)
            if (self.root_object is None): 
                oo = trailer.get_object("Root")
                if (oo is not None): 
                    self.root_object = (Utils.asObjectOrNull(self.get_object(oo.id0_), PdfDictionary))
            d = 0
            prev = Utils.asObjectOrNull(trailer.get_object("Prev"), PdfIntValue)
            if (prev is None): 
                break
            disp = prev.val
