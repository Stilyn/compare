...
class PdfDictionary(PdfObject):
    ...
    def get_object(self, key : str) -> 'PdfObject':
        res = None
        wrapres167 = RefOutArgWrapper(None)
        inoutres168 = Utils.tryGetValue(self.__m_items, key, wrapres167)
        res = wrapres167.value
        if (not inoutres168): 
            return None
        if (isinstance(res, PdfReference)): 
    ...
    def get_all_pages(self, res : typing.List['PdfDictionary']) -> None:
        ...
        if (dic is not None): 
            ...
            kids = Utils.asObjectOrNull(self.get_object("Kids"), PdfArray)
            if (kids is not None): 
                i = 0
    ...
    def extract_data(self) -> bytearray:
        ...
        if (dparms is not None): 
            iv = Utils.asObjectOrNull(dparms.get_object("Predictor"), PdfIntValue)
            if (iv is not None and iv.val > 1): 
                predict = iv.val
            iv = (Utils.asObjectOrNull(dparms.get_object("Columns"), PdfIntValue))
            if (iv is not None): 
                columns = iv.val
            iv = (Utils.asObjectOrNull(dparms.get_object("Colors"), PdfIntValue))
            if (iv is not None): 
                n = iv.val
    ...
    def get_total_data_stream(self, key : str) -> bytearray:
        ...
        first_pass563 = True
        while True:
            if first_pass563: first_pass563 = False
            else: i += 1
    ...
    @staticmethod
    def _new220(_arg1 : int) -> 'PdfDictionary':
        res = PdfDictionary()
