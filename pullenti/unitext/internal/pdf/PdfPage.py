...
class PdfPage(PdfRect):
    ...
    def __init__(self, dic_ : 'PdfDictionary', state0 : object=None) -> None:
        ...
        self.items = list()
        self.dic = dic_
        crop = Utils.asObjectOrNull(dic_.get_object("CropBox"), PdfArray)
        if (crop is None): 
            crop = (Utils.asObjectOrNull(dic_.get_object("TrimBox"), PdfArray))
        if (crop is None): 
            crop = (Utils.asObjectOrNull(dic_.get_object("MediaBox"), PdfArray))
        if (crop is None): 
            crop = (Utils.asObjectOrNull(dic_.get_object("BBox"), PdfArray))
        if (crop is None): 
            if (dic_.source_file.root_object is not None): 
                pages = dic_.source_file.root_object.get_dictionary("Pages", None)
                if (pages is not None): 
                    crop = (Utils.asObjectOrNull(pages.get_object("MediaBox"), PdfArray))
                    if (crop is None): 
                        crop = (Utils.asObjectOrNull(pages.get_object("TrimBox"), PdfArray))
                    if (crop is None): 
                        crop = (Utils.asObjectOrNull(pages.get_object("CropBox"), PdfArray))
        if (crop is not None and crop.items_count == 4): 
            self.x2 = crop.get_item(2).get_double()
            self.y2 = crop.get_item(3).get_double()
            rot = dic_.get_object("Rotate")
        ...
        if (state is None): 
            ...
            mat = Utils.asObjectOrNull(dic_.get_object("Matrix"), PdfArray)
        ...
        first_pass572 = True
        while True:
            if first_pass572: first_pass572 = False
            else: i += 1
                ...
                if (xobj is None): 
                    for rr in dic_.source_file.all_resources: 
                        if (rr.get_string_item("Name") == nam): 
                            ...
                            break
                if (xobj is not None): 
                    sub = Utils.asObjectOrNull(xobj.get_object("Subtype"), PdfName)
                    if (sub is not None and sub.name == "Image"): 
                        im = Utils.asObjectOrNull(xobj.get_object("ImageMask"), PdfBoolValue)
        ...
        first_pass573 = True
        while True:
            if first_pass573: first_pass573 = False
            else: i += 1
            if (not (i < len(self.items))): break
            txt1 = Utils.asObjectOrNull(self.items[i], PdfText)
            if (txt1 is None): 
                continue
            txt2 = None
            j = 0
            ...
            while j < len(self.items): 
                txt2 = Utils.asObjectOrNull(self.items[j], PdfText)
                if ((txt2) is not None): 
                    break
                j += 1
            if (txt2 is None): 
            ...
            first_pass574 = True
            while True:
                if first_pass574: first_pass574 = False
                else: k += 1
                if (not (k < j)): break
                txt0 = Utils.asObjectOrNull(self.items[k], PdfText)
                if (txt0 is None): 
                    continue
                if (txt2.y2 <= (txt0.y1 + (2))): 
                    for jj in range(j, (k + 1) - 1, -1):
                        self.items[jj] = self.items[jj - 1]
                    self.items[k] = (txt2)
                    break