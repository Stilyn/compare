...
class PdfStream(object):
    ...
    def try_read_id_and_version(self, must_be_ref : bool, id0_ : int, vers : int) -> bool:
        ...
        vers.value = (0)
        p0 = self.__m_stream.position
        inoutres214 = Utils.tryParseInt(self.read_word(False), id0_)
        if (not inoutres214): 
            self.__m_stream.position = p0
            return False
        inoutres213 = Utils.tryParseInt(self.read_word(False), vers)
        if (not inoutres213): 
            self.__m_stream.position = p0
            return False
    ...
    def parse_object(self, file : 'PdfFile', text : bool) -> 'PdfObject':
        ...
        if ((chr(ch)) >= '0' and (chr(ch)) <= '9' and not text): 
            id0_ = 0
            ver = 0
            wrapid215 = RefOutArgWrapper(0)
            wrapver216 = RefOutArgWrapper(0)
            inoutres217 = self.try_read_id_and_version(True, wrapid215, wrapver216)
            id0_ = wrapid215.value
            ver = wrapver216.value
            if (inoutres217): 
                res = (PdfReference())
                ...
                res.version = ver
                return res
        if ((((chr(ch)) >= '0' and (chr(ch)) <= '9')) or (chr(ch)) == '+' or (chr(ch)) == '-'): 
            str0_ = self.read_word(False)
            if (str0_ is None): 
                return None
            if (str0_.find('.') < 0): 
                v = 0
                wrapv218 = RefOutArgWrapper(0)
                inoutres219 = Utils.tryParseInt(str0_, wrapv218)
                v = wrapv218.value
                if (not inoutres219): 
                    return None
                ires = PdfIntValue()
