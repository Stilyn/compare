...
class RtfHelper:
    ...
    class RtfEncoder:
        ...
        @staticmethod
        def _new228(_arg1 : int, _arg2 : 'EncodingWrapper') -> 'RtfEncoder':
    ...
    class RtfItemProxy(RtfItem):
        ...
        @staticmethod
        def _new261(_arg1 : 'RftItemTyp') -> 'RtfItemProxy':
    ...
    def __manage_encodings(items : typing.List['RtfItem']) -> None:
        ...
        first_pass575 = True
        while True:
            if first_pass575: first_pass575 = False
            else: i += 1
            ...
            if (cmd.startswith("ansicpg")): 
                cp = 0
                wrapcp229 = RefOutArgWrapper(0)
                inoutres230 = Utils.tryParseInt(cmd[7:], wrapcp229)
                cp = wrapcp229.value
                if (not inoutres230): 
                    continue
                defs.insert(0, RtfHelper.RtfEncoder._new228(it.level, EncodingWrapper(EncodingStandard.UNDEFINED, None, cp)))
                continue
            if (cmd.startswith("deflangfe")): 
                cp = 0
                wrapcp234 = RefOutArgWrapper(0)
                inoutres235 = Utils.tryParseInt(cmd[9:], wrapcp234)
                cp = wrapcp234.value
                if (not inoutres235): 
                    continue
                wrapcp232 = RefOutArgWrapper(0)
                inoutres233 = Utils.tryGetValue(RtfHelper.M_LANG_ID_TO_CODEPAGE, cp, wrapcp232)
                cp = wrapcp232.value
                if (not inoutres233): 
                    continue
                defs.insert(0, RtfHelper.RtfEncoder._new228(it.level, EncodingWrapper(EncodingStandard.UNDEFINED, None, cp)))
            ...
            if (cmd.startswith("deflang")): 
                cp = 0
                wrapcp239 = RefOutArgWrapper(0)
                inoutres240 = Utils.tryParseInt(cmd[7:], wrapcp239)
                cp = wrapcp239.value
                if (not inoutres240): 
                    continue
                wrapcp237 = RefOutArgWrapper(0)
                inoutres238 = Utils.tryGetValue(RtfHelper.M_LANG_ID_TO_CODEPAGE, cp, wrapcp237)
                cp = wrapcp237.value
                if (not inoutres238): 
                    continue
                defs.insert(0, RtfHelper.RtfEncoder._new228(it.level, EncodingWrapper(EncodingStandard.UNDEFINED, None, cp)))
            ...
            if (cmd.startswith("lang")): 
                cp = 0
                wrapcp244 = RefOutArgWrapper(0)
                inoutres245 = Utils.tryParseInt(cmd[4:], wrapcp244)
                cp = wrapcp244.value
                if (not inoutres245): 
                    continue
                wrapcp242 = RefOutArgWrapper(0)
                inoutres243 = Utils.tryGetValue(RtfHelper.M_LANG_ID_TO_CODEPAGE, cp, wrapcp242)
                cp = wrapcp242.value
                if (not inoutres243): 
                    continue
                if ((i > 0 and items[i - 1].typ == RftItemTyp.COMMAND and items[i - 1].text.startswith("f")) and len(curs) > 0 and curs[0].level == it.level): 
                    continue
                curs.insert(0, RtfHelper.RtfEncoder._new228(it.level, EncodingWrapper(EncodingStandard.UNDEFINED, None, cp)))
                continue
            if (len(cmd) >= 2 and cmd[0] == 'f' and str.isdigit(cmd[1])): 
                ...
                if (cmd in font_charsets): 
                    ...
                    if (enc is not None): 
                        curs.insert(0, RtfHelper.RtfEncoder._new228(it.level, enc))
                else: 
                    font_name = cmd
                continue
            if (cmd.startswith("fcharset") and font_name is not None): 
                cod = 0
                wrapcod247 = RefOutArgWrapper(0)
                inoutres248 = Utils.tryParseInt(cmd[8:], wrapcod247)
                cod = wrapcod247.value
                if (inoutres248): 
    ...
    def _create_uni_doc(stream : Stream, pars : 'CreateDocumentParam') -> 'UnitextDocument':
        ...
        first_pass576 = True
        while True:
            if first_pass576: first_pass576 = False
            else: i += 1
            if (not (i < len(items))): break
            it = items[i]
            if (it.typ == RftItemTyp.COMMAND): 
        ...
        if (html_ is None): 
            ...
            if (head_gen is not None): 
                doc.header = head_gen.finish(True, None)
            if (foot_gen is not None): 
                doc.footer = foot_gen.finish(True, None)
        else: 
    ...
    def __manage_uni(items : typing.List['RtfItem'], html_ : io.StringIO, gen : 'UnitextGen', i0 : int, i1 : int, in_tab : bool, lev : int) -> int:
        ...
        first_pass577 = True
        while True:
            if first_pass577: first_pass577 = False
            else: i += 1
            if (not (i < i1)): break
            it = items[i]
            ...
            if (isinstance(it, RtfItemImage)): 
                ...
                img = UnitextImage._new250(im.codes)
                ...
                while i < len(items): 
                    if (items[i].level < it.level): 
                        ...
                        if (cmd == "*"): 
                            ...
                            img = UnitextImage._new250(items[i].codes)
                            if (im.width > 0 and im.height > 0): 
                                img.width = "{0}pt".format(im.width)
                                img.height = "{0}pt".format(im.height)
                            gen.append(img, None, -1, False)
                ...
                while ii < len(items): 
                    if (items[ii].level < it.level): 
                        ...
                        if (items[ii].text is not None and items[ii].text.startswith("SYMBOL")): 
                            ...
                            if (jj > 0): 
                                sss = sss[0:0+jj]
                            wrapjj252 = RefOutArgWrapper(0)
                            inoutres253 = Utils.tryParseInt(sss, wrapjj252)
                            jj = wrapjj252.value
                            if (inoutres253): 
                                if ("Wingdings" in items[ii].text): 
                                    ...
                                    gen.append_text("{0}".format(chr(jj)), False)
                    ii += 1
                i = (ii - 1)
                continue
            if (cmd == "field"): 
                ...
                while ii < len(items): 
                    if (items[ii].level < it.level): 
                        ...
                        if (items[ii].typ == RftItemTyp.TEXT and items[ii].text is not None and items[ii].text.strip().startswith("HYPERLINK")): 
                            fff = items[ii].text
                            jj0 = fff.find('"')
                            jj1 = Utils.indexOfList(fff, '"', jj0 + 1)
                ...
                if (cnt is not None and ur is not None): 
                    tmp = io.StringIO()
                    cnt.get_plaintext(tmp, GetPlaintextParam())
                    if (tmp.tell() > 300): 
                        gen.append(cnt, None, -1, False)
                    else: 
                        hr = UnitextHyperlink._new51(ur)
            ...
            if (cmd == "annotation"): 
                ...
                if (cnt is not None): 
                    ...
                    if (tmp.tell() > 0): 
                        gen.append(UnitextComment._new42(Utils.toStringStringIO(tmp)), None, -1, False)
                i = (ii - 1)
                continue
            if (cmd == "shptxt"): 
                iii = list()
                ...
                if (isinstance(cnt, UnitextContainer)): 
                    ...
                    cnt2 = UnitextContainer._new87(UnitextContainerType.SHAPE)
                    cnt2.children.append(cnt)
                    cnt = cnt2._optimize(False, None)
                if (cnt is not None): 
                    gen.append(cnt, None, -1, False)
                i = (ii - 1)
                continue
            if (cmd == "footnote"): 
                ...
                cnt = gg.finish(False, None)
                if (cnt is not None): 
                    fn = UnitextFootnote._new257(cnt, endnote)
                    if (isinstance(cnt, UnitextContainer)): 
                        ...
                        if (len(cc.children) > 1 and (isinstance(cc.children[0], UnitextPlaintext)) and cc.children[0].typ == UnitextPlaintextType.SUP): 
                            ...
                            del cc.children[0]
                            fn.content = cc._optimize(True, None)
            ...
            if (cmd.startswith("listtext")): 
                ...
                first_pass578 = True
                while True:
                    if first_pass578: first_pass578 = False
                    else: ii += 1
                    ...
                    if (itt.text.startswith("ilvl") and len(itt.text) > 4 and str.isdigit(itt.text[4])): 
                        wraplist_level258 = RefOutArgWrapper(0)
                        Utils.tryParseInt(itt.text[4:], wraplist_level258)
                        list_level = wraplist_level258.value
                        continue
                ...
                if (ok): 
                    ...
                    first_pass579 = True
                    while True:
                        if first_pass579: first_pass579 = False
                        else: jj += 1
            ...
            if (cmd == "brdrt" or cmd == "brdrb"): 
                if (html_ is not None): 
                    ...
                    gen.append(UnitextMisc._new259(UnitextMiscType.HORIZONTALLINE), None, -1, False)
                continue
            ...
            if (sub >= 0): 
                ...
                if (ok): 
                    ...
                    if (len(tt) > 0 and (len(tt) < 10)): 
                        gen.append(UnitextPlaintext._new50(tt, (UnitextPlaintextType.SUB if sub > 0 else UnitextPlaintextType.SUP)), None, -1, False)
                        i = (ii - 1)
                continue
        return i
        ...
        if (res.cols_count > 1): 
            ...
            for kk in range(3):
                for rr in tbl.children: 
                    ...
                    first_pass580 = True
                    while True:
                        if first_pass580: first_pass580 = False
                        else: i += 1
    ...
    def __manage_table_row(items : typing.List['RtfItem'], i : int, rtf_row : 'RichTextRow', lev : int) -> int:
        ...
        while i < len(items): 
            ...
            first_pass581 = True
            while True:
                if first_pass581: first_pass581 = False
                else: j += 1
                ...
                if (itt.text == "nestcell"): 
                    ...
                    if (jj > 0): 
                        ...
                        while True:
                            ...
                            jj = jjj
                        ntab._correct()
                        ppp = RtfHelper.RtfItemProxy._new261(RftItemTyp.PROXY)
    ...
    def __manage_nested_table_row(items : typing.List['RtfItem'], i : int, rtf_row : 'RichTextRow', lev : int) -> int:
        ...
        while i < len(items): 
            ...
            first_pass582 = True
            while True:
                if first_pass582: first_pass582 = False
                else: j += 1
                if (not (j < len(items))): break
                itt = items[j]
                if (itt.typ != RftItemTyp.COMMAND): 
        ...
        first_pass583 = True
        while True:
            if first_pass583: first_pass583 = False
            else: i += 1
            if (not (i < len(items))): break
            it = items[i]
            if (it.typ != RftItemTyp.COMMAND): 
