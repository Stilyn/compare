...
class RichTextRow(RichTextItem):
    ...
    class CellInfo:
        ...
        @staticmethod
        def _new222(_arg1 : int) -> 'CellInfo':
    ...
    def _add_cmd(self, cmd : str) -> bool:
        ...
        if (cmd.startswith("cellx")): 
            idd = 0
            wrapidd223 = RefOutArgWrapper(0)
            inoutres224 = Utils.tryParseInt(cmd[5:], wrapidd223)
            idd = wrapidd223.value
            if (inoutres224): 
                if (idd > 0): 
                    ...
                    if (last is not None and last.id0_ == 0): 
                        ...
                        self._cells_info.append(RichTextRow.CellInfo._new222(idd))
            return True
        if (cmd.startswith("clwWidth")): 
            wi = 0
            wrapwi225 = RefOutArgWrapper(0)
            inoutres226 = Utils.tryParseInt(cmd[8:], wrapwi225)
            wi = wrapwi225.value
            if (inoutres226): 
