...
class RtfItem:
    ...
    class RftCommand:
        ...
        def parse(self, stream : Stream) -> bool:
            ...
            if (self.command == "tab"): 
                ...
                wrapi262 = RefOutArgWrapper(0)
                inoutres263 = Utils.tryParseInt(self.command[1:], wrapi262)
                i = wrapi262.value
                if (inoutres263): 
                        ...
                        if (pict_comm.parse(stream)): 
                            if (pict_comm.command is not None): 
                                if (pict_comm.command.startswith("picwgoal")): 
                                    wrappic_goalw264 = RefOutArgWrapper(0)
                                    Utils.tryParseInt(pict_comm.command[8:], wrappic_goalw264)
                                    pic_goalw = wrappic_goalw264.value
                                elif (pict_comm.command.startswith("pichgoal")): 
                                    wrappic_goalh265 = RefOutArgWrapper(0)
                                    Utils.tryParseInt(pict_comm.command[8:], wrappic_goalh265)
                                    pic_goalh = wrappic_goalh265.value
                                elif (pict_comm.command.startswith("picw") or pict_comm.command.startswith("objw")): 
                                    wrappicw266 = RefOutArgWrapper(0)
                                    Utils.tryParseInt(pict_comm.command[4:], wrappicw266)
                                    picw = wrappicw266.value
                                elif (pict_comm.command.startswith("pich") or pict_comm.command.startswith("objh")): 
                                    wrappich267 = RefOutArgWrapper(0)
                                    Utils.tryParseInt(pict_comm.command[4:], wrappich267)
                                    pich = wrappich267.value
                                elif (pict_comm.command.startswith("picscalex")): 
                                    wrapscalx268 = RefOutArgWrapper(0)
                                    Utils.tryParseInt(pict_comm.command[9:], wrapscalx268)
                                    scalx = wrapscalx268.value
                                elif (pict_comm.command.startswith("picscaley")): 
                                    wrapscaly269 = RefOutArgWrapper(0)
                                    Utils.tryParseInt(pict_comm.command[9:], wrapscaly269)
                                    scaly = wrapscaly269.value
                                elif (pict_comm.command.startswith("bin")): 
                                    blen = 0
                                    wrapblen270 = RefOutArgWrapper(0)
                                    inoutres271 = Utils.tryParseInt(pict_comm.command[3:], wrapblen270)
                                    blen = wrapblen270.value
                                    if (inoutres271): 
    ...
    def parse_list(stream : Stream) -> typing.List['RtfItem']:
        ...
        while stream.position < stream.length:
            ...
            new_char = chr(0)
            if (ch == '{'): 
                new_item = RtfItem._new272(RftItemTyp.BRACKETOPEN)
            elif (ch == '}'): 
                new_item = RtfItem._new272(RftItemTyp.BRACKETCLOSE)
            elif (ch == '\\'): 
                ...
                if (command.char_code != (0)): 
                    ...
                    new_item = (RtfItemImage._new274(RftItemTyp.IMAGE, command.pic_width, command.pic_height, command.command, command.pict_content))
                elif (command.command is not None): 
                    new_item = RtfItem._new275(RftItemTyp.COMMAND, command.command)
            else: 
                if ((ord(ch)) == 0xD or (ord(ch)) == 0xA): 
                    continue
                if ((ord(ch)) < 0x80): 
            ...
            if (new_item is not None or new_code != (0)): 
                if (txt_buf.tell() > 0): 
                    res.append(RtfItem._new275(RftItemTyp.TEXT, Utils.toStringStringIO(txt_buf)))
                    Utils.setLengthStringIO(txt_buf, 0)
            if (new_item is not None or (ord(new_char)) != 0): 
                if (len(cod_buf) > 0): 
                    res.append(RtfItem._new277(RftItemTyp.TEXT, bytearray(cod_buf)))
                    cod_buf.clear()
    ...
    @staticmethod
    def _new272(_arg1 : 'RftItemTyp') -> 'RtfItem':
        res = RtfItem()
        res.typ = _arg1
        return res
    
    @staticmethod
    def _new275(_arg1 : 'RftItemTyp', _arg2 : str) -> 'RtfItem':
        res = RtfItem()
        res.typ = _arg1
        res.text = _arg2
        return res
    ...
    @staticmethod
    def _new277(_arg1 : 'RftItemTyp', _arg2 : bytearray) -> 'RtfItem':
        res = RtfItem()
        res.typ = _arg1
        res.codes = _arg2
        return res