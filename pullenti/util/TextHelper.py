...
class TextHelper:
    ...
    def check_encoding(file_name : str) -> 'EncodingWrapper':
        ...
        i = 0
        wrapi534 = RefOutArgWrapper(0)
        inoutres535 = EncodingWrapper.check_encoding(buf, wrapi534)
        i = wrapi534.value
        return inoutres535
    ...
    def read_string_from_bytes(data : bytearray, utf8 : bool=False) -> str:
        ...
        hlen = 0
        wraphlen536 = RefOutArgWrapper(0)
        enc = EncodingWrapper.check_encoding(data, wraphlen536)
        hlen = wraphlen536.value
        if (enc is None): 
            return None
    ...
    def extract_text(file_name : str, content : bytearray=None, unzip_archives : bool=True) -> str:
        ...
        if (frmc == FileFormatClass.ARCHIVE and not unzip_archives): 
            return None
        if (frmc == FileFormatClass.IMAGE): 
            return None
        doc = UnitextService.create_document(file_name, content, CreateDocumentParam._new537(True, not unzip_archives))
        if (doc is None): 
            return None
        return UnitextHelper.get_plaintext(doc)