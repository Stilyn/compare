...
class MiscHelper:
    ...
    def try_parse_float(str0_ : str, res : float) -> bool:
        ...
        if (Utils.isNullOrEmpty(str0_)): 
            return False
        inoutres526 = Utils.tryParseFloat(str0_, res)
        if (inoutres526): 
            return True
        inoutres525 = Utils.tryParseFloat(str0_.replace(',', '.'), res)
        if (str0_.find(',') >= 0 and inoutres525): 
            return True
        inoutres524 = Utils.tryParseFloat(str0_.replace('.', ','), res)
        if (str0_.find('.') >= 0 and inoutres524): 
            return True
        return False
    ...
    def try_parse_double(str0_ : str, res : float) -> bool:
        ...
        if (Utils.isNullOrEmpty(str0_)): 
            return False
        inoutres529 = Utils.tryParseFloat(str0_, res)
        if (inoutres529): 
            return True
        inoutres528 = Utils.tryParseFloat(str0_.replace(',', '.'), res)
        if (str0_.find(',') >= 0 and inoutres528): 
            return True
        inoutres527 = Utils.tryParseFloat(str0_.replace('.', ','), res)
        if (str0_.find('.') >= 0 and inoutres527): 
            return True
        return False
    ...
    def encode_string1251(str0_ : str) -> bytearray:
        ...
        while j < len(str0_): 
            ...
            if (i < 0x80): 
                ...
                b = 0
                wrapb530 = RefOutArgWrapper(0)
                inoutres531 = Utils.tryGetValue(MiscHelper.__m_utf_1251, i, wrapb530)
                b = wrapb530.value
                if (inoutres531): 
                    res[j] = b
                else: 
    ...
    def encode_string1252(str0_ : str) -> bytearray:
        ...
        while j < len(str0_): 
            ...
            if ((i < 0x80) or i >= 0xA0): 
                ...
                b = 0
                wrapb532 = RefOutArgWrapper(0)
                inoutres533 = Utils.tryGetValue(MiscHelper.__m_utf_1252, i, wrapb532)
                b = wrapb532.value
                if (inoutres533): 
                    res[j] = b
                else: 
    ...
    def decode_string1251(dat : bytearray, pos : int=0, len0_ : int=-1) -> str:
        ...
        while (j < (pos + len0_)) and (j < len(dat)): 
            ...
            if (i < 0x80): 
                print(chr(i), end="", file=tmp)
            elif (MiscHelper.__m_1251_utf[i] == 0): 
                print('?', end="", file=tmp)
            else: 
                print(chr(MiscHelper.__m_1251_utf[i]), end="", file=tmp)
            j += 1
        return Utils.toStringStringIO(tmp)
    ...
    def decode_string1252(dat : bytearray, pos : int=0, len0_ : int=-1) -> str:
        ...
        while (j < (pos + len0_)) and (j < len(dat)): 
            ...
            if ((i < 0x80) or i >= 0xA0): 
                print(chr(i), end="", file=tmp)
            elif (MiscHelper.__m_1252_utf[i] == 0): 
                print('?', end="", file=tmp)
            else: 
                print(chr(MiscHelper.__m_1252_utf[i]), end="", file=tmp)
            j += 1
        return Utils.toStringStringIO(tmp)
    ...
    def _static_ctor():
        ...
        MiscHelper.__m_1252_utf = Utils.newArray(256, 0)
        MiscHelper.__m_utf_1252 = dict()
        for i in range(0x100):
            MiscHelper.__m_1252_utf[i] = i
        for i in range(0x20):
            if (m_1252_80_9f[i] != 0): 
                MiscHelper.__m_1252_utf[i + 0x80] = m_1252_80_9f[i]
