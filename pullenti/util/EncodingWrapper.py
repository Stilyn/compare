...
class EncodingWrapper:
    ...
    def __init__(self, typ : 'EncodingStandard', str0_ : str=None, cp : int=0) -> None:
        ...
        if (ss.startswith("WINDOWS")): 
            cod = 0
            wrapcod519 = RefOutArgWrapper(0)
            inoutres520 = Utils.tryParseInt(ss[7:], wrapcod519)
            cod = wrapcod519.value
            if (inoutres520): 
                self.code_page = cod
                if (cod == 1251): 
                    self.std_typ = EncodingStandard.CP1251
        ...
        if (ss.startswith("CP")): 
            cod = 0
            wrapcod521 = RefOutArgWrapper(0)
            inoutres522 = Utils.tryParseInt(ss[2:], wrapcod521)
            cod = wrapcod521.value
            if (inoutres522): 
                self.code_page = cod
                if (cod == 1251): 
                    self.std_typ = EncodingStandard.CP1251
