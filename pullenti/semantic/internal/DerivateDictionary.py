...
class DerivateDictionary:
    ...
    def __load_tree_node(self, tn : 'ExplanTreeNode') -> None:
        with self._m_lock: 
            pos = tn.lazy_pos
            if (pos > 0): 
                wrappos2928 = RefOutArgWrapper(pos)
                tn._deserialize(self.__m_buf, self, True, wrappos2928)
                pos = wrappos2928.value
            tn.lazy_pos = 0
    ...
    def _deserialize(self, str0_ : Stream, lazy_load : bool) -> None:
        ...
        with MemoryStream() as tmp: 
            ...
            pos = 0
            wrappos2932 = RefOutArgWrapper(pos)
            cou = wr.deserialize_int(wrappos2932)
            pos = wrappos2932.value
            while cou > 0: 
                wrappos2930 = RefOutArgWrapper(pos)
                p1 = wr.deserialize_int(wrappos2930)
                pos = wrappos2930.value
                ew = DerivateGroup()
                if (lazy_load): 
                    ew._lazy_pos = pos
                    pos = p1
                else: 
                    wrappos2929 = RefOutArgWrapper(pos)
                    ew._deserialize(wr, wrappos2929)
                    pos = wrappos2929.value
                ew.id0_ = (len(self._m_all_groups) + 1)
                self._m_all_groups.append(ew)
                cou -= 1
            self._m_root = ExplanTreeNode()
            wrappos2931 = RefOutArgWrapper(pos)
            self._m_root._deserialize(wr, self, lazy_load, wrappos2931)
            pos = wrappos2931.value
        self.__m_buf = wr
