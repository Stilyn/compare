...
class AdverbToken(MetaToken):
        ...
        if (t.is_value("ДРУГ", None) or t.is_value("САМ", None)): 
            ...
            if (t1 is not None): 
                if (t1.is_value("ДРУГ", None) and t.is_value("ДРУГ", None)): 
                    return AdverbToken._new2893(t0, t1, SemAttributeType.EACHOTHER)
                if (t1.is_value("СЕБЯ", None) and t.is_value("САМ", None)): 
                    return AdverbToken._new2893(t0, t1, SemAttributeType.HIMELF)
        tok = AdverbToken.__m_termins.try_parse(t, TerminParseAttr.NO)
        if (tok is not None): 
            res = AdverbToken._new2893(t0, tok.end_token, Utils.valToEnum(tok.termin.tag, SemAttributeType))
    ...
    @staticmethod
    def _new2893(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'SemAttributeType') -> 'AdverbToken':
