...
class Processor(object):
    ...
    def find_analyzer(self, name : str) -> 'Analyzer':
        ...
        """
        a = None
        wrapa2845 = RefOutArgWrapper(None)
        inoutres2846 = Utils.tryGetValue(self.__m_analyzers_hash, Utils.ifNotNull(name, ""), wrapa2845)
        a = wrapa2845.value
        if (not inoutres2846): 
            return None
        else: 
    ...
    def process_next(self, ar : 'AnalysisResult') -> None:
        ...
        if (ar is None): 
            return
        kit = AnalysisKit._new2847(self, ar.ontology)
        kit._init_from(ar)
    ...
    def _process(self, text : 'SourceOfAnalysis', onto_regine : bool, no_log : bool, ext_ontology : 'ExtOntology'=None, lang : 'MorphLang'=None) -> 'AnalysisResult':
        ...
        kit = AnalysisKit._new2848(text, False, lang, self.__progress_changed_event_handler_on_progress_handler, ext_ontology, self, onto_regine)
        ar = AnalysisResult()
        sw0.stop()
        msg = None
    ...
    def __prepare_progress(self) -> None:
        with self.__m_progress_peaces_lock: 
            ...
            max0_ = co * 100
            max0_ /= (total)
            self.__m_progress_peaces[self] = ProgressPeace._new2849(0, max0_)
            for wf in self.analyzers: 
                min0_ = max0_
                ...
                if (not wf in self.__m_progress_peaces): 
                    self.__m_progress_peaces[wf] = ProgressPeace._new2849(min0_, max0_)
    ...
    def _on_progress_handler(self, sender : object, e0_ : ProgressEventArgs) -> None:
        ...
        if (e0_.progressPercentage >= 0): 
            pi0_ = None
            with self.__m_progress_peaces_lock: 
                wrappi2851 = RefOutArgWrapper(None)
                inoutres2852 = Utils.tryGetValue(self.__m_progress_peaces, Utils.ifNotNull(sender, self), wrappi2851)
                pi0_ = wrappi2851.value
                if (inoutres2852): 
    ...
    def manage_referent_links(self) -> None:
        if (self.__m_refs is not None): 
            for pr in self.__m_refs: 
                r = None
                wrapr2855 = RefOutArgWrapper(None)
                inoutres2856 = Utils.tryGetValue(self.__m_links2, pr.identity, wrapr2855)
                r = wrapr2855.value
                if (pr.identity is not None and self.__m_links2 is not None and inoutres2856): 
                    pr.owner_referent.upload_slot(pr.owner_slot, r)
                else: 
                    wrapr2853 = RefOutArgWrapper(None)
                    inoutres2854 = Utils.tryGetValue(self.__m_links, pr.value, wrapr2853)
                    r = wrapr2853.value
                    if (self.__m_links is not None and inoutres2854): 
                        pr.owner_referent.upload_slot(pr.owner_slot, r)
                    else: 
                        pass
        self.__m_links2 = None
        ...
        try: 
            ...
            for x in xml0_: 
                ...
                if (att is not None and att[1] == "true"): 
                    pr = ProxyReferent._new2857(Utils.getXmlInnerText(x), res)
                ...
                if ((att) is not None): 
                    cou = 0
                    wrapcou2858 = RefOutArgWrapper(0)
                    inoutres2859 = Utils.tryParseInt(att[1], wrapcou2858)
                    cou = wrapcou2858.value
                    if (inoutres2859): 
                        slot.count = cou
