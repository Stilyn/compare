...
class NumberingHelper:
    ...
    def calc_delta(prev : 'InstrToken1', next0_ : 'InstrToken1', can_sub_numbers : bool) -> int:
        ...
        if ((len(prev.numbers) - 1) == len(next0_.numbers) and len(prev.numbers) > 1): 
            ...
            inoutres1564 = Utils.tryParseInt(prev.numbers[len(prev.numbers) - 2], wrapn11563)
            n1 = wrapn11563.value
            if (not inoutres1564): 
        ...
        if ((len(prev.numbers) - 2) == len(next0_.numbers) and len(prev.numbers) > 2): 
            ...
            while i < (len(prev.numbers) - 3): 
                if (prev.numbers[i] != next0_.numbers[i]): 
                    return 0
                i += 1
            wrapn11565 = RefOutArgWrapper(0)
            inoutres1566 = Utils.tryParseInt(prev.numbers[len(prev.numbers) - 3], wrapn11565)
            n1 = wrapn11565.value
            if (not inoutres1566): 
                return 0
            if ((n1 + 1) != n2): 
        ...
        if ((len(prev.numbers) - 3) == len(next0_.numbers) and len(prev.numbers) > 3): 
            ...
            inoutres1568 = Utils.tryParseInt(prev.numbers[len(prev.numbers) - 4], wrapn11567)
            n1 = wrapn11567.value
            if (not inoutres1568): 
                return 0
            if ((n1 + 1) != n2): 
    ...
    def create_number(owner : 'FragToken', itok : 'InstrToken1') -> None:
        if (itok.num_begin_token is None or itok.num_end_token is None): 
            return
        num = FragToken._new1569(itok.num_begin_token, itok.num_end_token, InstrumentKind.NUMBER, True, itok)
    ...
    def create_diap(s1 : str, s2 : str) -> typing.List[str]:
        ...
        if (s2.startswith(s1)): 
            ...
            if (((i + 1) < len(s2)) and s2[i] == '.' and str.isdigit(s2[i + 1])): 
                wrapn21570 = RefOutArgWrapper(0)
                inoutres1571 = Utils.tryParseInt(s2[i + 1:], wrapn21570)
                n2 = wrapn21570.value
                if (inoutres1571): 
                    res0 = list()
                    res0.append(s1)
        ...
        if (((i)) > 0): 
            pref = s1[0:0+i + 1]
            wrapn11574 = RefOutArgWrapper(0)
            inoutres1575 = Utils.tryParseInt(s1[i + 1:], wrapn11574)
            n1 = wrapn11574.value
            if (not inoutres1575): 
                return None
            if (not s2.startswith(pref)): 
                return None
            wrapn21572 = RefOutArgWrapper(0)
            inoutres1573 = Utils.tryParseInt(s2[i + 1:], wrapn21572)
            n2 = wrapn21572.value
            if (not inoutres1573): 
                return None
        else: 
            wrapn11578 = RefOutArgWrapper(0)
            inoutres1579 = Utils.tryParseInt(s1, wrapn11578)
            n1 = wrapn11578.value
            if (not inoutres1579): 
                return None
            wrapn21576 = RefOutArgWrapper(0)
            inoutres1577 = Utils.tryParseInt(s2, wrapn21576)
            n2 = wrapn21576.value
            if (not inoutres1577): 
                return None
        if (n2 <= n1): 
