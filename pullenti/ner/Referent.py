...
class Referent:
    ...
    def get_int_value(self, attr_name : str, def_value : int) -> int:
        ...
        if (Utils.isNullOrEmpty(str0_)): 
            return def_value
        res = 0
        wrapres2865 = RefOutArgWrapper(0)
        inoutres2866 = Utils.tryParseInt(str0_, wrapres2865)
        res = wrapres2865.value
        if (not inoutres2866): 
            return def_value
        return res
    ...
    def add_occurence(self, anno : 'TextAnnotation') -> None:
        ...
        if (anno.occurence_of != self and anno.occurence_of is not None): 
            anno = TextAnnotation._new2868(anno.begin_char, anno.end_char, anno.sofa)
        if (self.__m_occurrence is None): 
            self.__m_occurrence = list()
    ...
    def clone(self) -> 'Referent':
        ...
        for r in self.slots: 
            rr = Slot._new2869(r.type_name, r.value, r.count)
            rr.owner = res
            res.slots.append(rr)
        return res
    ...
    def deserialize(self, stream : Stream, all0_ : typing.List['Referent'], sofa : 'SourceOfAnalysis') -> None:
        ...
        i = 0
        while i < cou: 
            a = TextAnnotation._new2870(sofa, self)
