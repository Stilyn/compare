...
class TitlePageAnalyzer(Analyzer):
    ...
    def process_referent1(self, begin : 'Token', end : 'Token') -> 'ReferentToken':
        et = None
        wrapet2678 = RefOutArgWrapper(None)
        tpr = TitlePageAnalyzer._process(begin, (0 if end is None else end.end_char), begin.kit, wrapet2678)
        et = wrapet2678.value
        if (tpr is None): 
            return None
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        et = None
        wrapet2679 = RefOutArgWrapper(None)
        tpr = TitlePageAnalyzer._process(kit.first_token, 0, kit, wrapet2679)
        et = wrapet2679.value
        if (tpr is not None): 
            ...
            for r in rli: 
                ...
                if (isinstance(r, OrganizationReferent)): 
                    ...
                    if ("курс" in org0_.types and org0_.number is not None): 
                        i = 0
                        wrapi2680 = RefOutArgWrapper(0)
                        inoutres2681 = Utils.tryParseInt(org0_.number, wrapi2680)
                        i = wrapi2680.value
                        if (inoutres2681): 
                            if (i > 0 and (i < 8)): 
                                res.student_year = i
