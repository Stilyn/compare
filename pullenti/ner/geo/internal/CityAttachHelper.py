...
class CityAttachHelper:
    ...
    def try_attach_city(li : typing.List['CityItemToken'], ad : 'AnalyzerDataWithOntology', always : bool=False) -> 'ReferentToken':
        ...
        if (res is None): 
            ...
            res = CityAttachHelper.__try1(li, wrapoi1132, ad)
            oi = wrapoi1132.value
        if (res is None): 
            wrapoi1133 = RefOutArgWrapper(None)
            res = CityAttachHelper.__try_noun_name(li, wrapoi1133, False)
            oi = wrapoi1133.value
        if (res is None): 
            wrapoi1134 = RefOutArgWrapper(None)
            res = CityAttachHelper.__try_name_exist(li, wrapoi1134, False)
            oi = wrapoi1134.value
        if (res is None): 
            res = CityAttachHelper.__try4(li)
        if (res is None and always): 
            wrapoi1135 = RefOutArgWrapper(None)
            res = CityAttachHelper.__try_noun_name(li, wrapoi1135, True)
            oi = wrapoi1135.value
        if (res is None and always): 
            if (AddressItemToken.try_attach_org(li[0].begin_token) is not None): 
                pass
            else: 
                wrapoi1136 = RefOutArgWrapper(None)
                res = CityAttachHelper.__try_name_exist(li, wrapoi1136, True)
                oi = wrapoi1136.value
        if (res is None): 
            return None
    ...
    def __try1(li : typing.List['CityItemToken'], oi : 'IntOntologyItem', ad : 'AnalyzerDataWithOntology') -> 'ReferentToken':
        ...
        if (oi.value is None): 
            ...
            rt = ReferentToken._new736(city, li[0].begin_token, li[len(li) - 1].end_token, mc)
        elif (typ is None): 
            typ = oi.value.typ
        if (rt is None): 
            city = GeoReferent()
    ...
    def __try_noun_name(li : typing.List['CityItemToken'], oi : 'IntOntologyItem', always : bool) -> 'ReferentToken':
        ...
        if (li[0].typ == CityItemToken.ItemType.MISC): 
            del li[0]
        res = ReferentToken._new736(city, li[0].begin_token, li[len(li) - 1].end_token, mc)
    ...
    def __try_name_exist(li : typing.List['CityItemToken'], oi : 'IntOntologyItem', always : bool) -> 'ReferentToken':
        ...
        if (oi.value is not None and (isinstance(oi.value.referent, GeoReferent)) and not oi.value.owner.is_ext_ontology): 
            ...
            if (not city.is_city): 
                city._add_typ_city(li[0].kit.base_language)
        return ReferentToken._new736(city, li[0].begin_token, li[0].end_token, li[0].morph)
