...
class TextToken(Token):
    ...
    def check_value(self, dict0_ : typing.List[tuple]) -> object:
        ...
        inoutres2876 = Utils.tryGetValue(dict0_, self.term, wrapres2875)
            ...
            for it in self.morph.items: 
                ...
                if (mf is not None): 
                    if (mf.normal_case is not None): 
                        ...
                        inoutres2872 = Utils.tryGetValue(dict0_, mf.normal_case, wrapres2871)
                        res = wrapres2871.value
                        if (inoutres2872): 
                            return res
                    if (mf.normal_full is not None and mf.normal_case != mf.normal_full): 
                        wrapres2873 = RefOutArgWrapper(None)
                        inoutres2874 = Utils.tryGetValue(dict0_, mf.normal_full, wrapres2873)
