...
class ExtOntology:
    ...
    def add(self, ext_id : object, type_name : str, definition_ : str) -> 'ExtOntologyItem':
        ...
        if (rs is None): 
            return None
        self.__m_hash = (None)
        res = ExtOntologyItem._new2814(ext_id, rs[0], type_name)
        if (len(rs) > 1): 
            del rs[0]
            res._refs = rs
    ...
    def add_referent(self, ext_id : object, referent : 'Referent') -> 'ExtOntologyItem':
        ...
        if (referent is None): 
            return None
        self.__m_hash = (None)
        res = ExtOntologyItem._new2814(ext_id, referent, referent.type_name)
        self.items.append(res)
        return res
    
    def __create_referent(self, type_name : str, definition_ : str) -> typing.List['Referent']:
        analyzer = None
        wrapanalyzer2816 = RefOutArgWrapper(None)
        inoutres2817 = Utils.tryGetValue(self.__m_anal_by_type, type_name, wrapanalyzer2816)
        analyzer = wrapanalyzer2816.value
        if (not inoutres2817): 
            return None
    ...
    def refresh(self, item : 'ExtOntologyItem', definition_ : object) -> bool:
        ...
        analyzer = None
        wrapanalyzer2818 = RefOutArgWrapper(None)
        inoutres2819 = Utils.tryGetValue(self.__m_anal_by_type, item.type_name, wrapanalyzer2818)
        analyzer = wrapanalyzer2818.value
        if (not inoutres2819): 
            return False
    ...
    def _get_analyzer_data(self, type_name : str) -> 'AnalyzerData':
        a = None
        wrapa2820 = RefOutArgWrapper(None)
        inoutres2821 = Utils.tryGetValue(self.__m_anal_by_type, type_name, wrapa2820)
        a = wrapa2820.value
        if (not inoutres2821): 
            return None
    ...
    def __init_hash(self) -> None:
        ...
        for it in self.items: 
            if (it.referent is not None): 
                ont = None
                wrapont2823 = RefOutArgWrapper(None)
                inoutres2824 = Utils.tryGetValue(self.__m_hash, it.referent.type_name, wrapont2823)
                ont = wrapont2823.value
                if (not inoutres2824): 
                    ont = IntOntologyCollection._new2822(True)
    ...
    def attach_referent(self, r : 'Referent') -> typing.List['ExtOntologyItem']:
        ...
        if (self.__m_hash is None): 
            self.__init_hash()
        onto = None
        wraponto2825 = RefOutArgWrapper(None)
        inoutres2826 = Utils.tryGetValue(self.__m_hash, r.type_name, wraponto2825)
        onto = wraponto2825.value
        if (not inoutres2826): 
            return None
    ...
    def attach_token(self, type_name : str, t : 'Token') -> typing.List['IntOntologyToken']:
        ...
        if (self.__m_hash is None): 
            self.__init_hash()
        onto = None
        wraponto2827 = RefOutArgWrapper(None)
        inoutres2828 = Utils.tryGetValue(self.__m_hash, type_name, wraponto2827)
        onto = wraponto2827.value
        if (not inoutres2828): 
            return None
