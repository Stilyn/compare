    ...
    def __try_attach_all_(self, token : 'Token', pars : 'TerminParseAttr'=TerminParseAttr.NO, main_root : bool=False) -> typing.List['TerminToken']:
        ...
        if (tt is not None): 
                ...
                if ((no_vars and tt.term0 is not None and tt.term != tt.term0) and tt.invariant_prefix_length_of_morph_vars <= len(tt.term0)): 
                    ...
                    while i < tt.invariant_prefix_length_of_morph_vars: 
                        ...
                        if (nod.children is None): 
                            ...
                            break
                        nn = None
                        wrapnn587 = RefOutArgWrapper(None)
                        inoutres588 = Utils.tryGetValue(nod.children, ch, wrapnn587)
                        nn = wrapnn587.value
                        if (not inoutres588): 
                            no_vars = True
                            break
            ...
            if (not no_vars): 
                ...
                inoutres594 = self.__manage_var(token, pars, s, nod, len0, wrapres593)
                ...
                while True:
                    ...
                    if (ok): 
                        ...
                        inoutres590 = self.__manage_var(token, pars, wf.normal_case, nod, tt.invariant_prefix_length_of_morph_vars, wrapres589)
                    ...
                    inoutres592 = self.__manage_var(token, pars, wf.normal_full, nod, tt.invariant_prefix_length_of_morph_vars, wrapres591)
                    res = wrapres591.value
                    if (inoutres592): 
                        was_vars = True
        elif (isinstance(token, NumberToken)): 
            wrapres595 = RefOutArgWrapper(res)
            inoutres596 = self.__manage_var(token, pars, str(token.value), root, 0, wrapres595)
        ...
        if (not was_vars and s is not None and len(s) == 1): 
            vars0_ = [ ]
            wrapvars597 = RefOutArgWrapper(None)
            inoutres598 = Utils.tryGetValue(self.__m_hash1, ord(s[0]), wrapvars597)
            vars0_ = wrapvars597.value
            if (inoutres598): 
                for t in vars0_: 
                    if (not t.lang.is_undefined): 
                        if (not token.morph.language.is_undefined): 
    ...
    def __manage_var(self, token : 'Token', pars : 'TerminParseAttr', v : str, nod : 'CharNode', i0 : int, res : typing.List['TerminToken']) -> bool:
        ...
        while i < len(v): 
            ...
            if (nod.children is None): 
                return False
            nn = None
            wrapnn599 = RefOutArgWrapper(None)
            inoutres600 = Utils.tryGetValue(nod.children, ch, wrapnn599)
            nn = wrapnn599.value
            if (not inoutres600): 
                return False
            nod = nn
    ...
    def find_termins_by_canonic_text(self, text : str) -> typing.List['Termin']:
        if (self.__m_hash_canonic is None): 
            ...
            for t in self.termins: 
                ct = t.canonic_text
                li = [ ]
                wrapli601 = RefOutArgWrapper(None)
                inoutres602 = Utils.tryGetValue(self.__m_hash_canonic, ct, wrapli601)
                li = wrapli601.value
                if (not inoutres602): 
                    li = list()
        ...
        inoutres604 = Utils.tryGetValue(self.__m_hash_canonic, text, wrapres603)
